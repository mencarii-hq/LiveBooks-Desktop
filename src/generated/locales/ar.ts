/* eslint-disable */
/* Auto-generated — do not edit. */
import type { LanguageMap } from 'utils/types';

const languageMap: LanguageMap = {
  '${0}': {
    translation: '${0}',
  },
  '${0} ${1} already exists.': {
    translation: '${0} ${1} موجودة من قبل.',
  },
  '${0} ${1} does not exist': {
    translation: '${0} ${1} غير موجود',
  },
  '${0} ${1} has been modified after loading please reload entry.': {
    translation: '${0} ${1} تم تعديله بعد التحميل يرجى إعادة تحميل الإدخال.',
  },
  '${0} ${1} is linked with existing records.': {
    translation: '${0} ${1} مرطبت بسجل موجود.',
  },
  '${0} account not set in Inventory Settings.': {
    translation: '${0} لم يتم تعيين الحساب في إعدادات المخزون.',
  },
  '${0} already applied.': {
    translation: 'تم تطبيق ${0} بالفعل.',
  },
  '${0} already saved': {
    translation: '${0} تم حفظ بالفعل',
  },
  '${0} already submitted': {
    translation: '${0} تم الارسال بالفعل',
  },
  '${0} before ${1}': {
    translation: '${0} قبل ${1}',
  },
  '${0} cancelled': {
    translation: '${0} تم الالغاء',
  },
  '${0} cannot be cancelled': {
    translation: '${0} لا يمكن الالغاء',
  },
  '${0} cannot be deleted': {
    translation: '${0} لا يمكن الحذف',
  },
  '${0} deleted': {
    translation: '${0} تم الحذف',
  },
  '${0} entries failed': {
    translation: '${0} فشل الادخال',
  },
  '${0} entries imported': {
    translation: '${0} الإدخالات المستوردة',
  },
  '${0} entry failed': {
    translation: '${0} فشل الإدخال',
  },
  '${0} entry imported': {
    translation: '${0} الإدخال مستورد',
  },
  '${0} fields selected': {
    translation: '${0} الحقول المحددة',
  },
  '${0} filters applied': {
    translation: '${0} تم تطبيق الفيلتر',
  },
  '${0} of type ${1} does not exist': {
    translation: '${0} من النوع ${1} غير موجود',
  },
  '${0} only has ${1} points': {
    translation: '${0} لديه ${1} نقطة فقط',
  },
  '${0} out of ${1}': {
    translation: '${0} من أصل ${1}',
  },
  '${0} party ${1} is different from ${2}': {
    translation: '${0} الطرف ${1} يختلف عن ${2}',
  },
  '${0} quantity ${1} added.': {
    translation: 'تمت إضافة كمية ${1} من ${0}.',
  },
  '${0} quantity 1 added.': {
    translation: 'تمت إضافة كمية ${0} عدد 1.',
  },
  '${0} row added.': {
    translation: 'تمت إضافة صف ${0}.',
  },
  '${0} rows': {
    translation: '${0} صفوف',
  },
  '${0} rows added.': {
    translation: 'تمت إضافة ${0} صفوف.',
  },
  '${0} saved': {
    translation: 'تم حفظ ${0}',
  },
  '${0} shortcuts': {
    translation: '${0} اختصارات',
  },
  '${0} stored at ${1}': {
    translation: 'تم تخزين ${0} في ${1}',
  },
  '${0} submitted': {
    translation: 'تم تسجيل ${0}',
  },
  '${0} value ${1} does not exist.': {
    translation: 'قيمة ${0} ${1} غير موجودة.',
  },
  '0%': {
    translation: '0%',
  },
  '03-23-2022': {
    translation: '03-23-2022',
  },
  '03/23/2022': {
    translation: '03/23/2022',
  },
  '1 filter applied': {
    translation: 'تم تطبيق فلتر واحد',
  },
  '2022-03-23': {
    translation: '2022-03-23',
  },
  '23 Mar, 2022': {
    translation: '23 Mar, 2022',
  },
  '23-03-2022': {
    translation: '23-03-2022',
  },
  '23.03.2022': {
    translation: '23.03.2022',
  },
  '23/03/2022': {
    translation: '23/03/2022',
  },
  '9888900000': {
    translation: '9888900000',
  },
  'A submittable entry is deleted only if it is in the cancelled state.': {
    translation: 'يُحذف الإدخال القابل للتسجيل فقط إذا كان في حالة ملغاة.',
  },
  'API Base URL': {
    translation: 'عنوان URL الأساسي لواجهة برمجة التطبيقات',
  },
  Account: {
    translation: 'الحساب',
  },
  'Account ${0} does not exist.': {
    translation: 'الحساب ${0} غير موجود.',
  },
  'Account Entries': {
    translation: 'إدخالات الحساب',
  },
  'Account Name': {
    translation: 'اسم الحساب',
  },
  'Account Type': {
    translation: 'نوع الحساب',
  },
  Accounting: {
    translation: 'المحاسبة',
  },
  'Accounting Entries': {
    translation: 'إدخالات المحاسبة',
  },
  'Accounting Ledger Entry': {
    translation: 'قيد دفتر الأستاذ المحاسبي',
  },
  'Accounting Settings': {
    translation: 'إعدادات المحاسبة',
  },
  Accounts: {
    translation: 'الحسابات',
  },
  'Accounts Payable': {
    translation: 'الحسابات الدائنة',
  },
  'Accounts Receivable': {
    translation: 'الحسابات المدينة',
  },
  'Accumulated Depreciation': {
    translation: 'الاستهلاك المتراكم',
  },
  Action: {
    translation: 'إجراء',
  },
  Active: {
    translation: 'نشط',
  },
  'Add Account': {
    translation: 'إضافة حساب',
  },
  'Add Customers': {
    translation: 'إضافة عملاء',
  },
  'Add Group': {
    translation: 'إضافة مجموعة',
  },
  'Add Items': {
    translation: 'إضافة أصناف',
  },
  'Add Row': {
    translation: 'إضافة صف',
  },
  'Add Suppliers': {
    translation: 'إضافة موردين',
  },
  'Add Taxes': {
    translation: 'إضافة ضرائب',
  },
  'Add a few customers to create your first sales invoice': {
    translation: 'أضف بعض العملاء لإنشاء أول فاتورة مبيعات لك',
  },
  'Add a few suppliers to create your first purchase invoice': {
    translation: 'أضف بعض الموردين لإنشاء أول فاتورة شراء لك',
  },
  'Add a filter': {
    translation: 'إضافة عامل تصفية',
  },
  'Add a remark': {
    translation: 'إضافة ملاحظة',
  },
  'Add attachment': {
    translation: 'إضافة مرفق',
  },
  'Add invoice terms': {
    translation: 'إضافة شروط الفاتورة',
  },
  'Add products or services that you buy from your suppliers': {
    translation: 'أضف المنتجات أو الخدمات التي تشتريها من الموردين',
  },
  'Add products or services that you sell to your customers': {
    translation: 'أضف المنتجات أو الخدمات التي تبيعها لعملائك',
  },
  'Add quote terms': {
    translation: 'إضافة شروط عرض السعر',
  },
  'Add transfer terms': {
    translation: 'إضافة شروط التحويل',
  },
  "Add'l Discounts": {
    translation: 'خصومات إضافية',
  },
  'Additional ${0} Serial Numbers required for ${1} quantity of ${2}.': {
    translation: 'هناك حاجة إلى ${0} أرقام تسلسلية إضافية لكمية ${1} من ${2}.',
  },
  'Additional quantity (${0}) required${1} to make outward transfer of item ${2} from ${3} on ${4}':
    {
      translation:
        'الكمية الإضافية (${0}) المطلوبة${1} لإجراء تحويل خارجي للصنف ${2} من ${3} بتاريخ ${4}',
    },
  Address: {
    translation: 'العنوان',
  },
  'Address Display': {
    translation: 'عرض العنوان',
  },
  'Address Line 1': {
    translation: 'سطر العنوان 1',
  },
  'Address Line 2': {
    translation: 'سطر العنوان 2',
  },
  'Address Name': {
    translation: 'اسم العنوان',
  },
  'Administrative Expenses': {
    translation: 'مصاريف إدارية',
  },
  Alert: {
    translation: 'تنبيه',
  },
  All: {
    translation: 'الكل',
  },
  'Allow to bypass filters': {
    translation: 'السماح بتجاوز عوامل التصفية',
  },
  Amount: {
    translation: 'المبلغ',
  },
  'Amount Paid': {
    translation: 'المبلغ المدفوع',
  },
  'Amount: ${0} and writeoff: ${1} is less than the total amount allocated to references: ${2}.':
    {
      translation:
        'المبلغ: ${0} والشطب: ${1} أقل من إجمالي المبلغ المخصص للمراجع: ${2}.',
    },
  'Amount: ${0} is less than the total amount allocated to references: ${1}.': {
    translation: 'المبلغ: ${0} أقل من إجمالي المبلغ المخصص للمراجع: ${1}.',
  },
  Amounts: {
    translation: 'المبالغ',
  },
  'An entry is cancelled only if it is in the submitted state.': {
    translation: 'يُلغى الإدخال فقط إذا كان في حالة مسجَّلة.',
  },
  'An entry is submitted only if it is submittable and is in the saved state.':
    {
      translation:
        'يُسجَّل الإدخال فقط إذا كان قابلاً للتسجيل وفي حالة محفوظة.',
    },
  'An error occurred.': {
    translation: 'حدث خطأ',
  },
  And: {
    translation: 'و',
  },
  'Applicable when POS is open': {
    translation: 'ينطبق عند فتح نقطة البيع',
  },
  'Applicable when Quick Search is open': {
    translation: 'ينطبق عند فتح البحث السريع',
  },
  'Applicable when Template Builder is open': {
    translation: 'ينطبق عند فتح منشئ القوالب',
  },
  'Applicable when a entry is open in the Form view or Quick Edit view': {
    translation: 'ينطبق عند فتح إدخال في عرض النموذج أو عرض التحرير السريع',
  },
  'Applicable when the List View of an entry type is open': {
    translation: 'ينطبق عند فتح عرض القائمة لنوع الإدخال',
  },
  'Application of Funds (Assets)': {
    translation: 'تطبيق الأموال (الأصول)',
  },
  'Applied Coupon Codes': {
    translation: 'رموز القسائم المطبقة',
  },
  'Applied Items': {
    translation: 'الأصناف المطبقة',
  },
  Apply: {
    translation: 'تطبيق',
  },
  'Apply Discount After Tax': {
    translation: 'تطبيق الخصم بعد الضريبة',
  },
  'Apply Price List': {
    translation: 'تطبيق قائمة الأسعار',
  },
  'Apply and view changes made to the print template': {
    translation: 'تطبيق وعرض التغييرات التي تم إجراؤها على قالب الطباعة',
  },
  April: {
    translation: 'أبريل',
  },
  Arial: {
    translation: 'Arial',
  },
  'Ascending Order': {
    translation: 'ترتيب تصاعدي',
  },
  Asset: {
    translation: 'أصل',
  },
  'Attach Image': {
    translation: 'إرفاق صورة',
  },
  Attachment: {
    translation: 'المرفقات',
  },
  August: {
    translation: 'أغسطس',
  },
  'Auth Token': {
    translation: 'رمز المصادقة',
  },
  'Auto Payments': {
    translation: 'المدفوعات التلقائية',
  },
  'Auto Stock Transfer': {
    translation: 'النقل التلقائي للمخزون',
  },
  Autocomplete: {
    translation: 'الإكمال التلقائي',
  },
  'Available Loyalty Points': {
    translation: 'نقاط الولاء المتاحة',
  },
  Back: {
    translation: 'رجوع',
  },
  'Back Reference': {
    translation: 'مرجع خلفي',
  },
  'Bad import data, could not read file.': {
    translation: 'بيانات استيراد غير صحيحة، تعذر قراءة الملف.',
  },
  Balance: {
    translation: 'الرصيد',
  },
  'Balance Amount': {
    translation: 'مبلغ الرصيد',
  },
  'Balance Sheet': {
    translation: 'الميزانية العمومية',
  },
  Bank: {
    translation: 'بنك',
  },
  'Bank Accounts': {
    translation: 'حسابات بنكية',
  },
  'Bank Entry': {
    translation: 'ادخالات البنوك',
  },
  'Bank Name': {
    translation: 'اسم البنك',
  },
  'Bank Overdraft Account': {
    translation: 'حساب مصرفي السحب على المكشوف',
  },
  Barcode: {
    translation: 'الباركود',
  },
  'Barcode ${0} has an invalid length.': {
    translation: 'يحتوي الرمز الشريطي ${0} على طول غير صالح.',
  },
  'Barcode must be exactly 12 digits.': {
    translation: 'يجب أن يتكون الباركود من 12 رقمًا بالضبط.',
  },
  'Base Grand Total': {
    translation: 'إجمالي المبلغ الأساسي',
  },
  'Based On': {
    translation: 'استنادًا إلى',
  },
  Batch: {
    translation: 'دفعة',
  },
  'Batch ${0} does not belong to Item ${1}': {
    translation: 'الدفعة ${0} لا تنتمي إلى العنصر ${1}',
  },
  'Batch not set for row ${0}.': {
    translation: 'لم يتم تعيين الدفعة للصف ${0}.',
  },
  'Batch set for row ${0}.': {
    translation: 'تم تعيين الدفعة للصف ${0}.',
  },
  'Bill Created': {
    translation: 'تم إنشاء الفاتورة',
  },
  Billing: {
    translation: 'الفوترة',
  },
  Billion: {
    translation: 'مليار',
  },
  Black: {
    translation: 'أسود',
  },
  Blue: {
    translation: 'أزرق',
  },
  Both: {
    translation: 'كلاهما',
  },
  'Both From and To Location cannot be undefined': {
    translation: 'لا يمكن أن يكون كل من موقع الإرسال والاستلام غير محدد',
  },
  Buildings: {
    translation: 'المباني',
  },
  'Can Change Discount': {
    translation: 'يمكن تغيير الخصم',
  },
  'Can Change Rate': {
    translation: 'يمكن تغيير السعر',
  },
  Cancel: {
    translation: 'إلغاء',
  },
  'Cancel ${0}?': {
    translation: 'إلغاء ${0}؟',
  },
  'Cancel Button Colour': {
    translation: 'لون زر الإلغاء',
  },
  'Cancel or Delete an entry.': {
    translation: 'إلغاء أو حذف إدخال.',
  },
  Cancelled: {
    translation: 'ملغاة',
  },
  'Cannot Commit Error': {
    translation: 'لا يمكن تنفيذ الالتزام (خطأ)',
  },
  'Cannot Delete': {
    translation: 'لا يمكن الحذف',
  },
  'Cannot Delete Account': {
    translation: 'لا يمكن حذف الحساب',
  },
  'Cannot Export': {
    translation: 'لا يمكن التصدير',
  },
  'Cannot Import': {
    translation: 'لا يمكن الاستيراد',
  },
  'Cannot Open File': {
    translation: 'لا يمكن فتح الملف',
  },
  'Cannot add an item to a submitted invoice.': {
    translation: 'لا يمكن إضافة صنف إلى فاتورة مُرسلة.',
  },
  'Cannot cancel ${0} ${1} because of the following ${2}: ${3}': {
    translation: 'لا يمكن إلغاء ${0} ${1} بسبب ${2} التالي: ${3}',
  },
  'Cannot cancel ${0} because of the following ${1}: ${2}': {
    translation: 'لا يمكن إلغاء ${0} بسبب ${1} التالي: ${2}',
  },
  'Cannot delete ${0} "${1}" because of linked entries.': {
    translation: 'لا يمكن حذف ${0} "${1}" بسبب الإدخالات المرتبطة.',
  },
  'Cannot open file': {
    translation: 'لا يمكن فتح الملف',
  },
  'Cannot perform operation.': {
    translation: 'لا يمكن تنفيذ العملية.',
  },
  'Cannot read file': {
    translation: 'لا يمكن قراءة الملف',
  },
  'Capital Equipments': {
    translation: 'معدات رأس المال',
  },
  'Capital Stock': {
    translation: 'رأس المال',
  },
  Cash: {
    translation: 'نقد',
  },
  'Cash Denominations': {
    translation: 'الفئات النقدية',
  },
  'Cash Entry': {
    translation: 'قيد نقدي',
  },
  'Cash In Denominations': {
    translation: 'النقد بالفئات',
  },
  'Cash In Hand': {
    translation: 'نقد في اليد',
  },
  Cashflow: {
    translation: 'التدفق النقدي',
  },
  Ceil: {
    translation: 'تقريب لأعلى',
  },
  'Central Tax': {
    translation: 'الضرائب المركزية',
  },
  'Change File': {
    translation: 'تغيير الملف',
  },
  'Change Ref Type': {
    translation: 'تغير نوع المرجع',
  },
  'Changes made to settings will be visible on reload.': {
    translation:
      'التغييرات التي تم إجراؤها على الإعدادات ستظهر بعد إعادة التحميل.',
  },
  Chargeable: {
    translation: 'قابل للتحصيل',
  },
  'Chart Of Accounts Reviewed': {
    translation: 'مراجعة دليل الحسابات',
  },
  'Chart of Accounts': {
    translation: 'دليل الحسابات',
  },
  Check: {
    translation: 'تحقق',
  },
  'Check Digits': {
    translation: 'التحقق من الأرقام',
  },
  'City / Town': {
    translation: 'المدينة / البلدة',
  },
  Classic: {
    translation: 'كلاسيكي',
  },
  Clear: {
    translation: 'مسح',
  },
  'Clear Synced Data From ERPNext SyncQueue': {
    translation: 'مسح البيانات المتزامنة من قائمة مزامنة ERPNext',
  },
  'Clearance Date': {
    translation: 'تاريخ التخليص',
  },
  'Clearance Date not set.': {
    translation: 'تاريخ التخليص غير محدد.',
  },
  'Clicking continue will remove all the selected items.': {
    translation: 'سيؤدي النقر فوق "متابعة" إلى إزالة كافة العناصر المحددة.',
  },
  Close: {
    translation: 'إغلاق',
  },
  'Close POS Shift': {
    translation: 'إغلاق مناوبة نقطة البيع',
  },
  'Close Quick Search': {
    translation: 'إغلاق البحث السريع',
  },
  Closing: {
    translation: 'إغلاق',
  },
  'Closing ${0} Amount can not be negative.': {
    translation: 'لا يمكن أن يكون مبلغ الإغلاق ${0} سالبًا.',
  },
  'Closing (Cr)': {
    translation: 'إغلاق (دائن)',
  },
  'Closing (Dr)': {
    translation: 'إغلاق (مدين)',
  },
  'Closing Amount': {
    translation: 'مبلغ الإغلاق',
  },
  'Closing Amounts': {
    translation: 'مبالغ الاغلاق',
  },
  'Closing Cash': {
    translation: 'الاغلاق النقدي',
  },
  'Closing Cash In Denominations': {
    translation: 'إغلاق النقدية بالفئات',
  },
  'Closing Date': {
    translation: 'تاريخ الإغلاق',
  },
  Collapse: {
    translation: 'طي',
  },
  'Collection Factor': {
    translation: 'معامل التحصيل',
  },
  'Collection Rules': {
    translation: 'قواعد التحصيل',
  },
  Color: {
    translation: 'اللون',
  },
  Colour: {
    translation: 'لون',
  },
  'Commission on Sales': {
    translation: 'عمولة المبيعات',
  },
  Common: {
    translation: 'عام',
  },
  Company: {
    translation: 'الشركة',
  },
  'Company Logo': {
    translation: 'شعار الشركة',
  },
  'Company Name': {
    translation: 'اسم الشركة',
  },
  'Company Setup': {
    translation: 'إعداد الشركة',
  },
  Completed: {
    translation: 'مكتمل',
  },
  Condition: {
    translation: 'شرط',
  },
  'Consolidate Columns': {
    translation: 'دمج الأعمدة',
  },
  Contact: {
    translation: 'جهة الاتصال',
  },
  Contacts: {
    translation: 'بيانات الاتصال',
  },
  Contains: {
    translation: 'يحتوي',
  },
  Continue: {
    translation: 'متابعة',
  },
  'Continue submitting Sales Invoice?': {
    translation: 'هل تريد الاستمرار في تسجيل فاتورة المبيعات؟',
  },
  'Contra Entry': {
    translation: 'قيد عكسي',
  },
  'Conversion Error': {
    translation: 'خطأ في التحويل',
  },
  'Conversion Factor': {
    translation: 'عامل التحويل',
  },
  Converted: {
    translation: 'تم التحويل',
  },
  'Cost Of Goods Sold Acc.': {
    translation: 'حساب تكلفة البضاعة المباعة',
  },
  'Cost of Goods Sold': {
    translation: 'تكلفة البضائع المباعة',
  },
  'Could not connect to database file ${0}, please select the file manually': {
    translation:
      'تعذّر الاتصال بملف قاعدة البيانات ${0}، يرجى تحديد الملف يدويًا',
  },
  Count: {
    translation: 'العدد',
  },
  'Counter Cash Account': {
    translation: 'حساب النقدية في الخزينة',
  },
  Country: {
    translation: 'البلد',
  },
  'Country Code': {
    translation: 'رمز البلد',
  },
  'Country code used to initialize regional settings.': {
    translation: 'رمز البلد يُستخدم لتهيئة الإعدادات الإقليمية',
  },
  'Coupon ${0} is not applicable for applied items.': {
    translation: 'القسيمة ${0} غير قابلة للتطبيق على الأصناف المطبقة.',
  },
  'Coupon Code': {
    translation: 'رمز القسيمة',
  },
  'Coupon Name': {
    translation: 'اسم القسيمة',
  },
  Coupons: {
    translation: 'القسائم',
  },
  Courier: {
    translation: 'خدمة التوصيل',
  },
  'Cr.': {
    translation: 'دائن',
  },
  'Cr. ${0}': {
    translation: 'دائن ${0}',
  },
  Create: {
    translation: 'إنشاء',
  },
  'Create Demo': {
    translation: 'إنشاء نموذج تجريبي',
  },
  'Create Payment.': {
    translation: 'إنشاء دفعة.',
  },
  'Create Purchase': {
    translation: 'إنشاء مشتريات',
  },
  'Create Purchase Invoice': {
    translation: 'إنشاء فاتورة مشتريات',
  },
  'Create Sale': {
    translation: 'إنشاء مبيعات',
  },
  'Create Sales Invoice': {
    translation: 'إنشاء فاتورة مبيعات',
  },
  'Create a new company and store it on your computer': {
    translation: 'أنشئ شركة جديدة وخزّنها على جهازك',
  },
  'Create a new company or select an existing one from your computer': {
    translation: 'أنشئ شركة جديدة أو اختر شركة موجودة من جهازك',
  },
  'Create a new entry of the same type as the List View': {
    translation: 'أنشئ إدخالًا جديدًا من نفس النوع كما في عرض القائمة',
  },
  'Create new ${0} entry?': {
    translation: 'إنشاء إدخال ${0} جديد؟',
  },
  'Create your first purchase invoice from the created supplier': {
    translation: 'أنشئ أول فاتورة شراء من المورد الذي تم إنشاؤه',
  },
  'Create your first sales invoice for the created customer': {
    translation: 'أنشئ أول فاتورة مبيعات للعميل الذي تم إنشاؤه',
  },
  Created: {
    translation: 'تم الإنشاء',
  },
  'Created By': {
    translation: 'تم الإنشاء بواسطة',
  },
  'Creating Items and Parties': {
    translation: 'إنشاء الأصناف والأطراف',
  },
  'Creating Journal Entries': {
    translation: 'إنشاء قيود يومية',
  },
  'Creating Purchase Invoices': {
    translation: 'إنشاء فواتير المشتريات',
  },
  Credit: {
    translation: 'الائتمان',
  },
  'Credit Card Entry': {
    translation: 'إدخال بطاقة ائتمان',
  },
  'Credit Note': {
    translation: 'إشعار دائن',
  },
  Creditors: {
    translation: 'الدائنون',
  },
  Currency: {
    translation: 'العملة',
  },
  'Currency Name': {
    translation: 'اسم العملة',
  },
  Current: {
    translation: 'الحالي',
  },
  'Current Assets': {
    translation: 'الأصول المتداولة',
  },
  'Current Liabilities': {
    translation: 'الخصوم المتداولة',
  },
  'Custom Field': {
    translation: 'حقل مخصص',
  },
  'Custom Fields': {
    translation: 'حقول مخصصة',
  },
  'Custom Form': {
    translation: 'نموذج مخصص',
  },
  'Custom Hex': {
    translation: 'قيمة سداسية مخصصة',
  },
  Customer: {
    translation: 'العميل',
  },
  'Customer Created': {
    translation: 'تم إنشاء العميل',
  },
  'Customer Currency': {
    translation: 'عملة العميل',
  },
  'Customer has no loyalty points to redeem': {
    translation: 'ليس لدى العميل أي نقاط ولاء لاستبدالها',
  },
  Customers: {
    translation: 'العملاء',
  },
  Customizations: {
    translation: 'التخصيصات',
  },
  'Customize Form': {
    translation: 'تخصيص النموذج',
  },
  'Customize your invoices by adding a logo and address details': {
    translation: 'خصص فواتيرك بإضافة الشعار وتفاصيل العنوان',
  },
  'Dark mode': {
    translation: 'الوضع الداكن',
  },
  Dashboard: {
    translation: 'لوحة المعلومات',
  },
  Data: {
    translation: 'البيانات',
  },
  'Data Sync Interval (In Minutes)': {
    translation: 'فاصل مزامنة البيانات (بالدقائق)',
  },
  'Database Error': {
    translation: 'خطأ في قاعدة البيانات',
  },
  Date: {
    translation: 'التاريخ',
  },
  'Date Format': {
    translation: 'تنسيق التاريخ',
  },
  'Date Time': {
    translation: 'التاريخ والوقت',
  },
  Day: {
    translation: 'اليوم',
  },
  Debit: {
    translation: 'مدين',
  },
  'Debit Note': {
    translation: 'إشعار مدين',
  },
  Debtors: {
    translation: 'المدينون',
  },
  December: {
    translation: 'ديسمبر',
  },
  'Decrease print template display scale': {
    translation: 'تقليل مقياس عرض قالب الطباعة',
  },
  Default: {
    translation: 'افتراضي',
  },
  'Default Account': {
    translation: 'الحساب الافتراضي',
  },
  'Default Cash Denominations': {
    translation: 'الفئات النقدية الافتراضية',
  },
  'Default Location': {
    translation: 'الموقع الافتراضي',
  },
  Defaults: {
    translation: 'الإعدادات الافتراضية',
  },
  Delete: {
    translation: 'حذف',
  },
  'Delete ${0}?': {
    translation: 'حذف ${0}؟',
  },
  'Delete Account': {
    translation: 'حذف الحساب',
  },
  'Delete Failed': {
    translation: 'فشل الحذف',
  },
  'Delete Group': {
    translation: 'حذف المجموعة',
  },
  Delivered: {
    translation: 'تم التسليم',
  },
  Denomination: {
    translation: 'الفئة',
  },
  Depreciation: {
    translation: 'الإهلاك',
  },
  'Depreciation Entry': {
    translation: 'قيد الإهلاك',
  },
  Description: {
    translation: 'الوصف',
  },
  Details: {
    translation: 'التفاصيل',
  },
  'Device ID': {
    translation: 'معرف الجهاز',
  },
  'Device is offline. Please connect to a network to continue.': {
    translation: 'الجهاز غير متصل. يرجى الاتصال بالشبكة للمتابعة.',
  },
  'Difference Amount': {
    translation: 'مبلغ الفرق',
  },
  'Direct Expenses': {
    translation: 'المصروفات المباشرة',
  },
  'Direct Income': {
    translation: 'الدخل المباشر',
  },
  'Directory for database file ${0} does not exist, please select the file manually':
    {
      translation:
        'دليل ملف قاعدة البيانات ${0} غير موجود، يرجى تحديد الملف يدويًا',
    },
  Disabled: {
    translation: 'معطّل',
  },
  'Discount Account': {
    translation: 'حساب الخصم',
  },
  'Discount Account is not set.': {
    translation: 'لم يتم تعيين حساب الخصم.',
  },
  'Discount Amount': {
    translation: 'مبلغ الخصم',
  },
  'Discount Amount (${0}) cannot be greated than Amount (${1}).': {
    translation: 'لا يمكن أن يكون مبلغ الخصم (${0}) أكبر من المبلغ (${1}).',
  },
  'Discount Percent': {
    translation: 'نسبة الخصم',
  },
  'Discount Percent (${0}) cannot be greater than 100.': {
    translation: 'لا يمكن أن تكون نسبة الخصم (${0}) أكبر من 100.',
  },
  'Discount Percentage': {
    translation: 'نسبة الخصم',
  },
  'Discount Type': {
    translation: 'نوع الخصم',
  },
  'Discounted Amount': {
    translation: 'المبلغ بعد الخصم',
  },
  Discounts: {
    translation: 'الخصومات',
  },
  'Display Amount In Words': {
    translation: 'عرض المبلغ بالكلمات',
  },
  'Display Description In Invoice': {
    translation: 'عرض الوصف في الفاتورة',
  },
  'Display Doc': {
    translation: 'عرض المستند',
  },
  'Display Logo in Invoice': {
    translation: 'عرض الشعار في الفاتورة',
  },
  'Display Precision': {
    translation: 'دقة العرض',
  },
  'Display Precision should have a value between 0 and 9.': {
    translation: 'دقة العرض يجب أن تكون لها قيمة بين 0 و 9.',
  },
  'Display Scale': {
    translation: 'مقياس العرض',
  },
  'Display Terms and Conditions': {
    translation: 'عرض الشروط والأحكام',
  },
  'Display Time In Invoice': {
    translation: 'عرض الوقت في الفاتورة',
  },
  'Dividends Paid': {
    translation: 'توزيعات أرباح مدفوعة',
  },
  'Do not Contact': {
    translation: 'عدم الاتصال',
  },
  'Doc ${0} ${1} not set': {
    translation: 'لم يتم تعيين المستند ${0} ${1}',
  },
  Docs: {
    translation: 'المستندات',
  },
  'Document Name': {
    translation: 'اسم المستند',
  },
  Documentation: {
    translation: 'التوثيق',
  },
  'Does Not Contain': {
    translation: 'لا يحتوي',
  },
  Done: {
    translation: 'تم',
  },
  'Dr.': {
    translation: 'مدين',
  },
  'Dr. ${0}': {
    translation: 'مدين ${0}',
  },
  Draft: {
    translation: 'مسودة',
  },
  Duplicate: {
    translation: 'تكرار',
  },
  'Duplicate Entry': {
    translation: 'إدخال مكرر',
  },
  'Duplicate Template': {
    translation: 'قالب مكرر',
  },
  'Duplicate columns found: ${0}': {
    translation: 'تم العثور على أعمدة مكررة: ${0}',
  },
  'Duties and Taxes': {
    translation: 'الرسوم والضرائب',
  },
  'Dynamic Link': {
    translation: 'رابط ديناميكي',
  },
  'ERPNext Sync Queue': {
    translation: 'قائمة مزامنة ERPNext',
  },
  'ERPNext Sync Settings': {
    translation: 'إعدادات مزامنة ERPNext',
  },
  'Earnest Money': {
    translation: 'كسب المال',
  },
  Eight: {
    translation: 'ثمانية',
  },
  Eighteen: {
    translation: 'ثمانية عشر',
  },
  Eighty: {
    translation: 'ثمانون',
  },
  'Electronic Equipments': {
    translation: 'معدات إلكترونية',
  },
  Eleven: {
    translation: 'أحد عشر',
  },
  Email: {
    translation: 'البريد الإلكتروني',
  },
  'Email Address': {
    translation: 'عنوان البريد الإلكتروني',
  },
  Empty: {
    translation: 'فارغ',
  },
  'Empty file selected': {
    translation: 'تم اختيار ملف فارغ',
  },
  'Enable Barcodes': {
    translation: 'تفعيل الباركود',
  },
  'Enable Batches': {
    translation: 'تفعيل الدُفعات',
  },
  'Enable Coupon Code': {
    translation: 'تفعيل رمز القسيمة',
  },
  'Enable Discount Accounting': {
    translation: 'تفعيل محاسبة الخصومات',
  },
  'Enable ERPNext Sync': {
    translation: 'تفعيل المزامنة مع ERPNext',
  },
  'Enable Form Customization': {
    translation: 'تفعيل تخصيص النماذج',
  },
  'Enable Inventory': {
    translation: 'تفعيل المخزون',
  },
  'Enable Invoice Returns': {
    translation: 'تفعيل إرجاع الفواتير',
  },
  'Enable Item Enquiry': {
    translation: 'تفعيل استعلام الصنف',
  },
  'Enable Item Group': {
    translation: 'تفعيل مجموعة الأصناف',
  },
  'Enable Lead': {
    translation: 'تفعيل العملاء المحتملين',
  },
  'Enable Loyalty Program': {
    translation: 'تفعيل برنامج الولاء',
  },
  'Enable Partial Payment': {
    translation: 'تفعيل الدفع الجزئي',
  },
  'Enable Partial payment to pay partial amount': {
    translation: 'تمكين الدفع الجزئي لسداد جزء من المبلغ',
  },
  'Enable Point of Sale': {
    translation: 'تفعيل نقطة البيع',
  },
  'Enable Price List': {
    translation: 'تفعيل قائمة الأسعار',
  },
  'Enable Pricing Rule': {
    translation: 'تفعيل قاعدة التسعير',
  },
  'Enable Serial Number': {
    translation: 'تفعيل الأرقام التسلسلية',
  },
  'Enable UOM Conversion': {
    translation: 'تفعيل تحويل وحدات القياس',
  },
  Enabled: {
    translation: 'مفعل',
  },
  'Enabled For': {
    translation: 'مفعل لـ',
  },
  Endpoints: {
    translation: 'نقاط النهاية',
  },
  'Enter Country to load States': {
    translation: 'اختار الدولة حتى يتم تحميل  المدن',
  },
  'Enter State': {
    translation: 'ادخل  المدينة',
  },
  'Enter barcode': {
    translation: 'أدخل الباركود',
  },
  'Enter weight barcode': {
    translation: 'أدخل باركود الوزن',
  },
  'Entertainment Expenses': {
    translation: 'مصاريف الترفيه',
  },
  Entry: {
    translation: 'إدخال',
  },
  'Entry Currency': {
    translation: 'عملة الإدخال',
  },
  'Entry Label': {
    translation: 'تسمية الإدخال',
  },
  'Entry No': {
    translation: 'رقم الإدخال',
  },
  'Entry No.': {
    translation: 'رقم الإدخال.',
  },
  'Entry Type': {
    translation: 'نوع الإدخال',
  },
  'Entry has Grand Total ${0}. Please verify amounts.': {
    translation:
      'الإدخال يحتوي على المجموع الكلي ${0}. يرجى التحقق من المبالغ.',
  },
  Equity: {
    translation: 'حقوق الملكية',
  },
  Error: {
    translation: 'خطأ',
  },
  'Exchange Gain/Loss': {
    translation: 'مكاسب/خسائر الصرف',
  },
  'Exchange Rate': {
    translation: 'سعر الصرف',
  },
  'Excise Entry': {
    translation: 'إدخال الرسوم الضريبية',
  },
  'Existing Company': {
    translation: 'شركة موجودة',
  },
  Expand: {
    translation: 'توسيع',
  },
  'Expected Amount': {
    translation: 'المبلغ المتوقع',
  },
  Expense: {
    translation: 'المصروف',
  },
  'Expense Account': {
    translation: 'حساب المصروف',
  },
  Expenses: {
    translation: 'المصروفات',
  },
  'Expenses Included In Valuation': {
    translation: 'المصروفات المدرجة في التقييم',
  },
  'Expiry Date': {
    translation: 'تاريخ الانتهاء',
  },
  'Expiry Duration': {
    translation: 'مدة انتهاء الصلاحية',
  },
  Export: {
    translation: 'تصدير',
  },
  'Export Failed': {
    translation: 'فشل التصدير',
  },
  'Export Format': {
    translation: 'تنسيق التصدير',
  },
  'Export Successful': {
    translation: 'تم التصدير بنجاح',
  },
  'Export Wizard': {
    translation: 'معالج التصدير',
  },
  'FBooks Integration Version': {
    translation: 'إصدار تكامل FBooks',
  },
  Failed: {
    translation: 'فشل',
  },
  'Failed to load batches': {
    translation: 'فشل تحميل الدفعات',
  },
  Fax: {
    translation: 'فاكس',
  },
  Features: {
    translation: 'الخصائص',
  },
  February: {
    translation: 'فبراير',
  },
  'Fetch Data From ERPNext': {
    translation: 'جلب البيانات من ERPNext',
  },
  'Fetch From ERPNext Queue': {
    translation: 'جلب من قائمة ERPNext',
  },
  Field: {
    translation: 'الحقل',
  },
  Fieldname: {
    translation: 'اسم الحقل',
  },
  'Fieldname ${0} already exists for ${1}': {
    translation: 'اسم الحقل ${0} موجود بالفعل لـ ${1}',
  },
  'Fieldname ${0} already used for Custom Field ${1}': {
    translation: 'اسم الحقل ${0} مستخدم بالفعل للحقل المخصص ${1}',
  },
  Fieldtype: {
    translation: 'نوع الحقل',
  },
  Fifteen: {
    translation: 'خمسة عشر',
  },
  Fifty: {
    translation: 'خمسون',
  },
  'File ${0} does not exist.': {
    translation: 'الملف ${0} غير موجود.',
  },
  'File selection failed': {
    translation: 'فشل اختيار الملف',
  },
  Filter: {
    translation: 'التصفية',
  },
  'Filter by Group': {
    translation: 'تصفية حسب المجموعة',
  },
  'Fiscal Year': {
    translation: 'السنة المالية',
  },
  'Fiscal Year End Date': {
    translation: 'تاريخ نهاية السنة المالية',
  },
  'Fiscal Year Start Date': {
    translation: 'تاريخ بداية السنة المالية',
  },
  Five: {
    translation: 'خمسة',
  },
  'Fix Failed': {
    translation: 'فشل الإصلاح',
  },
  'Fixed Asset': {
    translation: 'أصل ثابت',
  },
  'Fixed Assets': {
    translation: 'الأصول الثابتة',
  },
  Float: {
    translation: 'عدد عشري',
  },
  Floor: {
    translation: 'تقريب لأسفل',
  },
  'Following cells have errors: ${0}.': {
    translation: 'الخلايا التالية تحتوي على أخطاء: ${0}.',
  },
  'Following links do not exist: ${absentLinks .map((l) =>': {
    translation: 'الروابط التالية غير موجودة: ${absentLinks .map((l) =>',
  },
  Font: {
    translation: 'الخط',
  },
  'For Purchase': {
    translation: 'للاستخدام في الشراء',
  },
  'For Sales': {
    translation: 'للاستخدام في المبيعات',
  },
  'Forbidden Error': {
    translation: 'خطأ غير مسموح',
  },
  'Form Section': {
    translation: 'قسم النموذج',
  },
  'Form Tab': {
    translation: 'تبويب النموذج',
  },
  'Form Type': {
    translation: 'نوع النموذج',
  },
  Forty: {
    translation: 'أربعون',
  },
  Four: {
    translation: 'أربعة',
  },
  Fourteen: {
    translation: 'أربعة عشر',
  },
  Fr: {
    translation: 'الجمعة',
  },
  Fraction: {
    translation: 'كسر',
  },
  'Fraction Units': {
    translation: 'وحدات الكسر',
  },
  'Free Item': {
    translation: 'صنف مجاني',
  },
  'Free item "${0}" was not added due to zero quantity': {
    translation: 'لم تتم إضافة الصنف المجاني "${0}" لأن الكمية تساوي صفرًا',
  },
  'Free items out of stock: ${0}': {
    translation: 'العناصر المجانية غير المتوفرة في المخزون: ${0}',
  },
  'Freight and Forwarding Charges': {
    translation: 'مصاريف الشحن',
  },
  From: {
    translation: 'من',
  },
  'From Account': {
    translation: 'من الحساب',
  },
  'From Date': {
    translation: 'من تاريخ',
  },
  'From Lead': {
    translation: 'من العميل المحتمل',
  },
  'From Loc.': {
    translation: 'من الموقع',
  },
  'From Year': {
    translation: 'من سنة',
  },
  'Full Name': {
    translation: 'الاسم الكامل',
  },
  'Furnitures and Fixtures': {
    translation: 'الأثاث والتجهيزات',
  },
  GST: {
    translation: 'ضريبة السلع والخدمات',
  },
  'GSTIN No.': {
    translation: 'رقم GSTIN',
  },
  GSTR1: {
    translation: 'إقرار ضريبة السلع والخدمات 1',
  },
  GSTR2: {
    translation: 'إقرار ضريبة السلع والخدمات 2',
  },
  'Gain/Loss on Asset Disposal': {
    translation: 'الربح / الخسارة من التصرف في الأصول',
  },
  General: {
    translation: 'عام',
  },
  'General Ledger': {
    translation: 'دفتر الأستاذ العام',
  },
  'Get Started': {
    translation: 'ابدأ',
  },
  Global: {
    translation: 'عالمي',
  },
  'Go back to the previous page': {
    translation: 'العودة إلى الصفحة السابقة',
  },
  Gram: {
    translation: 'غرام',
  },
  'Grand Total': {
    translation: 'المجموع الكلي',
  },
  'Greater Than': {
    translation: 'أكبر من',
  },
  Green: {
    translation: 'أخضر',
  },
  'Grid View': {
    translation: 'عرض الشبكة',
  },
  'Group By': {
    translation: 'تجميع حسب',
  },
  'HSN/SAC': {
    translation: 'HSN/SAC',
  },
  'HSN/SAC Code': {
    translation: 'رمز HSN/SAC',
  },
  'Half Yearly': {
    translation: 'نصف سنوي',
  },
  'Half Years': {
    translation: 'أنصاف السنوات',
  },
  'Has Batch': {
    translation: 'يحتوي على دفعة',
  },
  'Has Serial Number': {
    translation: 'يحتوي على رقم تسلسلي',
  },
  'Height (in cm)': {
    translation: 'الارتفاع (بالسنتيمتر)',
  },
  Held: {
    translation: 'معلّقة',
  },
  'Held Button Colour': {
    translation: 'لون زر التعليق',
  },
  Help: {
    translation: 'مساعدة',
  },
  'Hex Value': {
    translation: 'قيمة سداسية',
  },
  'Hidden values will be visible on Print on.': {
    translation: 'القيم المخفية ستظهر عند الطباعة.',
  },
  'Hide Get Started': {
    translation: 'إخفاء البدء',
  },
  'Hide Group Amounts': {
    translation: 'إخفاء مجاميع المبالغ',
  },
  'Hide Month/Year': {
    translation: 'إخفاء الشهر/السنة',
  },
  'Hide Unavailable Items': {
    translation: 'إخفاء الأصناف غير المتاحة',
  },
  'Hides the Get Started section from the sidebar. Change will be visible on restart or refreshing the app.':
    {
      translation:
        'يخفي قسم البدء من الشريط الجانبي. سيكون التغيير مرئيًا عند إعادة التشغيل أو تحديث التطبيق',
    },
  'Hold Q and type digits to set selected item quantity': {
    translation: 'استمر بالضغط على Q ثم اكتب الأرقام لتعيين كمية الصنف المحدد',
  },
  Hour: {
    translation: 'ساعة',
  },
  Hundred: {
    translation: 'مائة',
  },
  INR: {
    translation: 'روبية هندية',
  },
  'If any entry form is open, save the details. Otherwise, save the invoice.': {
    translation:
      'إذا كان نموذج الإدخال مفتوحًا، احفظ التفاصيل. وإلا، فاحفظ الفاتورة.',
  },
  'If any entry form is open, your entry will be canceled. Otherwise the selected items will be removed.':
    {
      translation:
        'إذا كان نموذج الإدخال مفتوحًا، فسيتم إلغاء الإدخال. وإلا فستُزال العناصر المحددة.',
    },
  'Ignore Pricing Rule': {
    translation: 'تجاهل قاعدة التسعير',
  },
  Image: {
    translation: 'صورة',
  },
  'Import Complete': {
    translation: 'اكتمل الاستيراد',
  },
  'Import Data': {
    translation: 'استيراد البيانات',
  },
  'Import Data.': {
    translation: 'استيراد البيانات.',
  },
  'Import Type': {
    translation: 'نوع الاستيراد',
  },
  'Import Wizard': {
    translation: 'معالج الاستيراد',
  },
  'Importer not set, reload tool': {
    translation: 'لم يتم تعيين المستورد، يرجى إعادة تحميل الأداة',
  },
  'In stock': {
    translation: 'متوفر في المخزون',
  },
  Inactive: {
    translation: 'غير نشط',
  },
  'Include Cancelled': {
    translation: 'تضمين الملغى',
  },
  Income: {
    translation: 'الدخل',
  },
  'Income Account': {
    translation: 'حساب الدخل',
  },
  'Increase print template display scale': {
    translation: 'زيادة مقياس عرض قالب الطباعة',
  },
  Indigo: {
    translation: 'نيلي',
  },
  'Indirect Expenses': {
    translation: 'مصاريف غير مباشرة',
  },
  'Indirect Income': {
    translation: 'دخل غير مباشر',
  },
  Inflow: {
    translation: 'التدفق الداخل',
  },
  'Initial Sync Data': {
    translation: 'بيانات المزامنة الأولية',
  },
  'Instance Id': {
    translation: 'معرّف المثيل',
  },
  'Insufficient Quantity': {
    translation: 'كمية غير كافية',
  },
  'Insufficient Quantity.': {
    translation: 'كمية غير كافية.',
  },
  'Insufficient Quantity. Item ${0} has only ${1} quantities available. you selected ${2}':
    {
      translation:
        'كمية غير كافية. الصنف ${0} يحتوي فقط على ${1} كمية متاحة. لقد اخترت ${2}',
    },
  Int: {
    translation: 'عدد صحيح',
  },
  'Integration Error Log': {
    translation: 'سجل أخطاء التكامل',
  },
  Interested: {
    translation: 'مهتم',
  },
  'Intergrated Tax': {
    translation: 'ضريبة متكاملة',
  },
  'Internal Precision': {
    translation: 'الدقة الداخلية',
  },
  'Invalid HSN Code.': {
    translation: 'رمز HSN غير صالح.',
  },
  'Invalid Key Error': {
    translation: 'خطأ في المفتاح',
  },
  'Invalid Quantity for Item ${0}': {
    translation: 'كمية غير صالحة للصنف ${0}',
  },
  'Invalid barcode value ${0}.': {
    translation: 'قيمة باركود غير صالحة ${0}.',
  },
  'Invalid value ${0} for ${1}': {
    translation: 'قيمة غير صالحة ${0} لـ ${1}',
  },
  'Invalid value found for ${0}': {
    translation: 'تم العثور على قيمة غير صالحة لـ ${0}',
  },
  Inventory: {
    translation: 'المخزون',
  },
  'Inventory Items': {
    translation: 'أصناف المخزون',
  },
  'Inventory Settings': {
    translation: 'إعدادات المخزون',
  },
  Investments: {
    translation: 'استثمارات',
  },
  Invoice: {
    translation: 'فاتورة',
  },
  'Invoice Created': {
    translation: 'الفاتورة المُنشأة',
  },
  'Invoice Date': {
    translation: 'تاريخ الفاتورة',
  },
  'Invoice Item': {
    translation: 'عنصر الفاتورة',
  },
  'Invoice No': {
    translation: 'رقم الفاتورة',
  },
  'Invoice No.': {
    translation: 'رقم الفاتورة',
  },
  'Invoice Value': {
    translation: 'قيمة الفاتورة',
  },
  Invoices: {
    translation: 'الفواتير',
  },
  Is: {
    translation: 'هو',
  },
  'Is Coupon Code Based': {
    translation: 'يعتمد على رمز القسيمة',
  },
  'Is Custom': {
    translation: 'مُخصّص',
  },
  'Is Empty': {
    translation: 'فارغ',
  },
  'Is Enabled': {
    translation: 'مفعّل',
  },
  'Is Group': {
    translation: 'مجموعة',
  },
  'Is Landscape': {
    translation: 'أفقي',
  },
  'Is Not': {
    translation: 'ليس',
  },
  'Is Not Empty': {
    translation: 'ليس فارغ',
  },
  'Is POS Shift Open': {
    translation: 'هل وردية نقطة البيع مفتوحة',
  },
  'Is Price List Enabled': {
    translation: 'هل قائمة الأسعار مفعّلة',
  },
  'Is Pricing Rule Enabled': {
    translation: 'هل قاعدة التسعير مفعّلة',
  },
  'Is Recursive': {
    translation: 'متكرر',
  },
  'Is Required': {
    translation: 'مطلوب',
  },
  'Is Sync Enabled': {
    translation: 'هل المزامنة مفعّلة',
  },
  'Is Whole': {
    translation: 'كامل',
  },
  Item: {
    translation: 'صنف',
  },
  'Item ${0} has Zero Quantity': {
    translation: 'الصنف ${0} كميته صفر',
  },
  'Item ${0} is a batched item': {
    translation: 'الصنف ${0} عنصر مجمّع',
  },
  'Item ${0} is not a batched item': {
    translation: 'الصنف ${0} ليس عنصرًا مجمّعًا',
  },
  'Item ${0} is out of stock (quantity is zero)': {
    translation: 'العنصر ${0} غير متوفر في المخزون (الكمية صفر)',
  },
  'Item ${0} not found': {
    translation: 'لم يتم العثور على العنصر ${0}',
  },
  'Item ${0} not in Stock': {
    translation: 'الصنف ${0} غير متوفر في المخزون',
  },
  'Item ${0} only has ${1} Quantity': {
    translation: 'يحتوي العنصر ${0} على كمية ${1} فقط',
  },
  'Item ${0} only has ${1} Quantity in batch ${2}': {
    translation: 'لا يتوفر من الصنف ${0} سوى كمية ${1} في الدفعة ${2}',
  },
  'Item Code': {
    translation: 'رمز الصنف',
  },
  'Item Code Digits': {
    translation: 'أرقام رمز العنصر',
  },
  'Item Description': {
    translation: 'وصف الصنف',
  },
  'Item Discounts': {
    translation: 'خصومات الصنف',
  },
  'Item Enquiry': {
    translation: 'استفسار عن الصنف',
  },
  'Item Group': {
    translation: 'مجموعة الصنف',
  },
  'Item Name': {
    translation: 'اسم الصنف',
  },
  'Item Prices': {
    translation: 'أسعار الصنف',
  },
  'Item Visibility': {
    translation: 'ظهور الصنف',
  },
  'Item enquiry submitted': {
    translation: 'تم إرسال استفسار الصنف',
  },
  'Item with From location not found': {
    translation: 'الصنف بموقع الإرسال غير موجود',
  },
  'Item with To location not found': {
    translation: 'الصنف بموقع الاستلام غير موجود',
  },
  'Item with barcode ${0} not found.': {
    translation: 'الصنف بالباركود ${0} غير موجود.',
  },
  ItemCode: {
    translation: 'رمز العنصر',
  },
  Items: {
    translation: 'العناصر',
  },
  January: {
    translation: 'يناير',
  },
  'John Doe': {
    translation: 'فلان الفلاني',
  },
  'Journal Entries': {
    translation: 'القيود اليومية',
  },
  'Journal Entry': {
    translation: 'قيد يومية',
  },
  'Journal Entry Account': {
    translation: 'حساب القيد اليومي',
  },
  'Journal Entry Number Series': {
    translation: 'سلسلة أرقام القيود اليومية',
  },
  'Journal Entry Print Template': {
    translation: 'قالب طباعة القيد اليومي',
  },
  July: {
    translation: 'يوليو',
  },
  June: {
    translation: 'يونيو',
  },
  'Key Hints': {
    translation: 'تلميحات المفاتيح',
  },
  Kg: {
    translation: 'كجم',
  },
  Label: {
    translation: 'تسمية',
  },
  Language: {
    translation: 'اللغة',
  },
  Lead: {
    translation: 'عميل محتمل',
  },
  'Left Index': {
    translation: 'الفهرس الأيسر',
  },
  'Legal Expenses': {
    translation: 'المصاريف القانونية',
  },
  'Less Filters': {
    translation: 'عوامل تصفية أقل',
  },
  'Less Than': {
    translation: 'أقل من',
  },
  Liability: {
    translation: 'الالتزام',
  },
  Limit: {
    translation: 'الحد',
  },
  Link: {
    translation: 'رابط',
  },
  'Link Validation Error': {
    translation: 'خطأ في التحقق من الرابط',
  },
  'Linked Entries': {
    translation: 'إدخالات مرتبطة',
  },
  List: {
    translation: 'قائمة',
  },
  'List View': {
    translation: 'عرض القائمة',
  },
  'Load an existing company from your computer': {
    translation: 'حمّل شركة موجودة من جهازك',
  },
  'Loading Report...': {
    translation: 'جاري تحميل التقرير...',
  },
  'Loading instance...': {
    translation: 'جاري تحميل المثيل...',
  },
  'Loading...': {
    translation: 'جاري التحميل...',
  },
  'Loans (Liabilities)': {
    translation: 'قروض (خصوم)',
  },
  'Loans and Advances (Assets)': {
    translation: 'القروض والسلف (الأصول)',
  },
  Locale: {
    translation: 'اللغة',
  },
  Location: {
    translation: 'الموقع',
  },
  'Location Name': {
    translation: 'اسم الموقع',
  },
  Logo: {
    translation: 'الشعار',
  },
  'Loyalty Point Entry': {
    translation: 'قيد نقاط الولاء',
  },
  'Loyalty Points': {
    translation: 'نقاط الولاء',
  },
  'Loyalty Points Redemption': {
    translation: 'استرداد نقاط الولاء',
  },
  'Loyalty Program': {
    translation: 'برنامج الولاء',
  },
  'Loyalty Program Tier': {
    translation: 'شريحة برنامج الولاء',
  },
  'Make Entry': {
    translation: 'ادخال جديد',
  },
  'Make Payment On Submit': {
    translation: 'إجراء الدفع عند الإرسال',
  },
  'Make Purchase Receipt On Submit': {
    translation: 'إنشاء إيصال شراء عند الإرسال',
  },
  'Make Shipment On Submit': {
    translation: 'إنشاء شحنة عند الإرسال',
  },
  'Mandatory Error': {
    translation: 'خطأ إلزامي',
  },
  Manufacture: {
    translation: 'التصنيع',
  },
  'Manufacture Date': {
    translation: 'تاريخ التصنيع',
  },
  'Mar 23, 2022': {
    translation: '23 مارس 2022',
  },
  March: {
    translation: 'مارس',
  },
  'Mark ${0} as submitted?': {
    translation: 'وضع علامة على ${0} كمُسجَّل؟',
  },
  'Marketing Expenses': {
    translation: 'مصاريف التسويق',
  },
  'Material Issue': {
    translation: 'صرف المواد',
  },
  'Material Receipt': {
    translation: 'استلام المواد',
  },
  'Material Transfer': {
    translation: 'تحويل المواد',
  },
  'Max Amount': {
    translation: 'أقصى مبلغ',
  },
  'Max Qty (As Per Stock Unit)': {
    translation: 'أقصى كمية (حسب وحدة المخزون)',
  },
  'Maximum Amount should be greater than the Minimum Amount.': {
    translation: 'يجب أن يكون الحد الأقصى للمبلغ أكبر من الحد الأدنى للمبلغ.',
  },
  "Maximum Amount should be lesser than Pricing Rule's Maximum Amount": {
    translation:
      'يجب أن يكون الحد الأقصى للمبلغ أقل من الحد الأقصى للمبلغ الخاص بقاعدة التسعير',
  },
  'Maximum Quantity should be greater than the Minimum Quantity.': {
    translation: 'يجب أن تكون الكمية القصوى أكبر من الكمية الدنيا.',
  },
  'Maximum Use': {
    translation: 'الحد الأقصى للاستخدام',
  },
  May: {
    translation: 'مايو',
  },
  Meter: {
    translation: 'متر',
  },
  Million: {
    translation: 'مليون',
  },
  'Min Amount': {
    translation: 'أدنى مبلغ',
  },
  'Min Qty (As Per Stock Unit)': {
    translation: 'أدنى كمية (حسب وحدة المخزون)',
  },
  "Minimum Amount should be greather than the Pricing Rule's Minimum Amount.": {
    translation:
      'يجب أن يكون الحد الأدنى للمبلغ أكبر من الحد الأدنى للمبلغ الخاص بقاعدة التسعير.',
  },
  'Minimum Amount should be less than the Maximum Amount.': {
    translation: 'يجب أن يكون الحد الأدنى للمبلغ أقل من الحد الأقصى للمبلغ.',
  },
  'Minimum Quantity should be less than the Maximum Quantity.': {
    translation: 'يجب أن يكون الحد الأدنى للكمية أقل من الحد الأقصى للكمية.',
  },
  'Minimum Total Spent': {
    translation: 'الحد الأدنى لإجمالي الإنفاق',
  },
  Misc: {
    translation: 'متفرقات',
  },
  Miscellaneous: {
    translation: 'متنوع',
  },
  'Miscellaneous Expenses': {
    translation: 'مصاريف متنوعة',
  },
  Mo: {
    translation: 'الاثنين',
  },
  Mobile: {
    translation: 'الهاتف المحمول',
  },
  Modern: {
    translation: 'حديث',
  },
  Modified: {
    translation: 'معدّل',
  },
  'Modified By': {
    translation: 'عُدّل بواسطة',
  },
  Monthly: {
    translation: 'شهري',
  },
  Months: {
    translation: 'أشهر',
  },
  More: {
    translation: 'المزيد',
  },
  'More Filters': {
    translation: 'المزيد من عوامل التصفية',
  },
  'More shortcuts will be added soon.': {
    translation: 'سيتم إضافة المزيد من الاختصارات قريبًا',
  },
  'Movement Type': {
    translation: 'نوع التنقل',
  },
  Name: {
    translation: 'الاسم',
  },
  Navigate: {
    translation: 'التنقل',
  },
  'Need ${0} Serial Numbers for Item ${1}. You have provided ${2}': {
    translation: 'تحتاج إلى ${0} أرقام تسلسلية للصنف ${1}. لقد قمت بتوفير ${2}',
  },
  'Net Total': {
    translation: 'الإجمالي الصافي',
  },
  'New ${0}': {
    translation: 'جديد ${0}',
  },
  'New ${0} ${1}': {
    translation: 'جديد ${0} ${1}',
  },
  'New Account': {
    translation: 'حساب جديد',
  },
  'New Company': {
    translation: 'شركة جديدة',
  },
  'New Entry': {
    translation: 'إدخال جديد',
  },
  'New Template': {
    translation: 'قالب جديد',
  },
  Nine: {
    translation: 'تسعة',
  },
  Nineteen: {
    translation: 'تسعة عشر',
  },
  Ninety: {
    translation: 'تسعون',
  },
  No: {
    translation: 'لا',
  },
  'No Display Entries Found': {
    translation: 'لم يتم العثور على إدخالات للعرض',
  },
  'No Print Templates not found for entry type ${0}': {
    translation: 'لم يتم العثور على قوالب طباعة لنوع الإدخال ${0}',
  },
  'No Value': {
    translation: 'لا توجد قيمة',
  },
  'No Values to be Displayed': {
    translation: 'لا توجد قيم للعرض',
  },
  'No entries found': {
    translation: 'لم يتم العثور على إدخالات',
  },
  'No entries were imported.': {
    translation: 'لم يتم استيراد أي إدخالات.',
  },
  'No expenses in this period': {
    translation: 'لا توجد نفقات في هذه الفترة',
  },
  'No filters selected': {
    translation: 'لم يتم تحديد عوامل تصفية',
  },
  'No linked entries found': {
    translation: 'لم يتم العثور على إدخالات مرتبطة',
  },
  'No results found': {
    translation: 'لم يتم العثور على نتائج',
  },
  'No results found, disable filters': {
    translation: 'لم يتم العثور على نتائج، عطّل عوامل التصفية',
  },
  'No rows added. Select a file or add rows.': {
    translation: 'لم تتم إضافة أي صفوف. اختر ملفًا أو أضف صفوفًا.',
  },
  'No transactions yet': {
    translation: 'لا توجد معاملات حتى الآن',
  },
  'Non Active Serial Number ${0} cannot be used as Manufacture raw material': {
    translation:
      'لا يمكن استخدام الرقم التسلسلي غير النشط ${0} كمادة خام في التصنيع',
  },
  'Non Active Serial Number ${0} cannot be used for Material Issue': {
    translation: 'لا يمكن استخدام الرقم التسلسلي غير النشط ${0} في صرف المواد',
  },
  'Non Active Serial Number ${0} cannot be used for Material Transfer': {
    translation:
      'لا يمكن استخدام الرقم التسلسلي غير النشط ${0} في تحويل المواد',
  },
  'Non Inactive Serial Number ${0} cannot be used for Material Receipt': {
    translation:
      'لا يمكن استخدام الرقم التسلسلي غير غير النشط ${0} في استلام المواد',
  },
  'Non-Inventory Items': {
    translation: 'أصناف غير مخزنية',
  },
  None: {
    translation: 'لا شيء',
  },
  'Not Found': {
    translation: 'غير موجود',
  },
  'Not Saved': {
    translation: 'غير محفوظ',
  },
  'Not Submitted': {
    translation: 'غير مسجّل',
  },
  'Not Transferred': {
    translation: 'غير مُحوَّل',
  },
  Notes: {
    translation: 'ملاحظات',
  },
  November: {
    translation: 'نوفمبر',
  },
  'Number Display': {
    translation: 'عرض الرقم',
  },
  'Number Series': {
    translation: 'سلسلة الأرقام',
  },
  'Number of ${0}': {
    translation: 'عدد ${0}',
  },
  'Number of Rows': {
    translation: 'عدد الصفوف',
  },
  October: {
    translation: 'أكتوبر',
  },
  'Office Equipments': {
    translation: 'معدات مكتبية',
  },
  'Office Maintenance Expenses': {
    translation: 'مصاريف صيانة المكاتب',
  },
  'Office Rent': {
    translation: 'إيجار مكتب',
  },
  Okay: {
    translation: 'حسنًا',
  },
  'Onboarding Complete': {
    translation: 'اكتمل الإعداد',
  },
  One: {
    translation: 'واحد',
  },
  'Only From or To can be set for Manufacture': {
    translation: 'يمكن تعيين من أو إلى فقط لعملية التصنيع',
  },
  Open: {
    translation: 'مفتوح',
  },
  'Open Count': {
    translation: 'فتح العدّ',
  },
  'Open Documentation': {
    translation: 'فتح التوثيق',
  },
  'Open Folder': {
    translation: 'فتح مجلد',
  },
  'Open POS Shift': {
    translation: 'فتح مناوبة نقطة البيع',
  },
  'Open Print View': {
    translation: 'فتح عرض الطباعة',
  },
  'Open Print View if Print is available.': {
    translation: 'فتح عرض الطباعة إذا كان الطباعة متاحة.',
  },
  'Open Quick Search': {
    translation: 'فتح البحث السريع',
  },
  'Open Report Print View': {
    translation: 'فتح عرض طباعة التقرير',
  },
  'Open Sales Invoice List': {
    translation: 'فتح قائمة فواتير المبيعات',
  },
  'Open Saved or Submitted Invoice List.': {
    translation: 'فتح قائمة الفواتير المحفوظة أو المسجّلة.',
  },
  'Open the Export Wizard modal': {
    translation: 'فتح نافذة معالج التصدير',
  },
  'Opening (Cr)': {
    translation: '(Cr) الافتتاح',
  },
  'Opening (Dr)': {
    translation: '(Dr) افتتاح',
  },
  'Opening Amount': {
    translation: 'المبلغ الافتتاحي',
  },
  'Opening Amounts': {
    translation: 'أرصدة افتتاحية',
  },
  'Opening Balance Equity': {
    translation: 'رصيد افتتاحي لحقوق الملكية',
  },
  'Opening Balances': {
    translation: 'الأرصدة الافتتاحية',
  },
  'Opening Cash': {
    translation: 'نقدية افتتاحية',
  },
  'Opening Cash Amount can not be negative.': {
    translation: 'لا يمكن أن يكون مبلغ النقدية الافتتاحي سالبًا.',
  },
  'Opening Cash In Denominations': {
    translation: 'النقدية الافتتاحية بالفئات',
  },
  'Opening Date': {
    translation: 'تاريخ الافتتاح',
  },
  'Opening Entry': {
    translation: 'القيد الافتتاحي',
  },
  'Opening Shift': {
    translation: 'وردية افتتاحية',
  },
  Opportunity: {
    translation: 'فرصة',
  },
  Options: {
    translation: 'الخيارات',
  },
  Orange: {
    translation: 'برتقالي',
  },
  Organisation: {
    translation: 'المؤسسة',
  },
  'Out stock': {
    translation: 'غير متوفر في المخزون',
  },
  Outflow: {
    translation: 'التدفق الخارج',
  },
  Outstanding: {
    translation: 'المتبقي',
  },
  'Outstanding Amount': {
    translation: 'المبلغ المستحق',
  },
  POS: {
    translation: 'نقطة البيع',
  },
  'POS Closing Shift': {
    translation: 'وردية إغلاق نقطة البيع',
  },
  'POS Counter Cash Account is not set. Please set it on POS Settings': {
    translation:
      'لم يتم تعيين حساب النقدية لنقطة البيع. يرجى تعيينه في إعدادات نقطة البيع.',
  },
  'POS Customer': {
    translation: 'عميل نقطة البيع',
  },
  'POS Inventory is not set. Please set it on POS Settings': {
    translation:
      'لم يتم تعيين مخزون نقطة البيع. يرجى تعيينه في إعدادات نقطة البيع.',
  },
  'POS Opening Shift': {
    translation: 'وردية افتتاح نقطة البيع',
  },
  'POS Print Template': {
    translation: 'قالب طباعة نقطة البيع',
  },
  'POS Profile': {
    translation: 'ملف تعريف نقطة البيع',
  },
  'POS Settings': {
    translation: 'إعدادات نقطة البيع',
  },
  'POS Shift Amount': {
    translation: 'مبلغ وردية نقطة البيع',
  },
  'POS UI': {
    translation: 'واجهة نقطة البيع',
  },
  'POS Write Off Account is not set. Please set it on POS Settings': {
    translation:
      'لم يتم تعيين حساب الإهلاك لنقطة البيع. يرجى تعيينه في إعدادات نقطة البيع.',
  },
  'Pad Zeros': {
    translation: 'إضافة أصفار',
  },
  Page: {
    translation: 'صفحة',
  },
  Paid: {
    translation: 'مدفوع',
  },
  'Paid ${0}': {
    translation: 'مدفوع ${0}',
  },
  'Paid Change': {
    translation: 'الباقي المدفوع',
  },
  Paisa: {
    translation: 'بيسة',
  },
  Parent: {
    translation: 'الأصل',
  },
  'Parent Account': {
    translation: 'الحساب الأصل',
  },
  'Partly Paid': {
    translation: 'مدفوع جزئيًا',
  },
  'Partly Paid ${0}': {
    translation: 'مدفوع جزئيًا ${0}',
  },
  Party: {
    translation: 'مورد / عميل',
  },
  'Patch Run': {
    translation: 'تشغيل التصحيح',
  },
  Pay: {
    translation: 'دفع',
  },
  'Pay & Print': {
    translation: 'الدفع والطباعة',
  },
  'Pay And Print Button Colour': {
    translation: 'لون زر الدفع والطباعة',
  },
  'Pay Button Colour': {
    translation: 'لون زر الدفع',
  },
  Payable: {
    translation: 'دائن',
  },
  Payment: {
    translation: 'الدفع',
  },
  'Payment ${0} is Saved': {
    translation: 'تم حفظ الدفع ${0}',
  },
  'Payment For': {
    translation: 'الدفع مقابل',
  },
  'Payment Method': {
    translation: 'طريقة الدفع',
  },
  'Payment No': {
    translation: 'رقم الدفع',
  },
  'Payment Number Series': {
    translation: 'تسلسل المدفوعات',
  },
  'Payment Print Template': {
    translation: 'قالب الطابعة المدفوعات',
  },
  'Payment Reference': {
    translation: 'مرجع الدفع',
  },
  'Payment Type': {
    translation: 'نوع الدفع',
  },
  'Payment amount cannot be ${0}.': {
    translation: 'لا يمكن أن يكون مبلغ الدفعة ${0}',
  },
  'Payment amount cannot be less than zero.': {
    translation: 'مبلغ الدفعة لا يمكن أن يكون أقل من الصفر.',
  },
  'Payment amount cannot exceed ${0}.': {
    translation: 'مبلغ الدفعة لا يمكن أن يتجاوز ${0} .',
  },
  'Payment amount: ${0} should be greater than 0.': {
    translation: 'مبلغ الدفعة: ${0} يجب أن يكون أكبر من 0.',
  },
  'Payment amount: ${0} should be less than Outstanding amount: ${1}.': {
    translation: 'مبلغ الدفعة: ${0} يجب أن يكون أقل من المبلغ المستحق: ${1}',
  },
  'Payment of ${0} will be made from account "${1}" to account "${2}" on Submit.':
    {
      translation:
        'سيتم دفع ${0} من الحساب "${1}" إلى الحساب "${2}" عند التسجيل.',
    },
  Payments: {
    translation: 'المدفوعات',
  },
  'Payroll Payable': {
    translation: 'الرواتب المستحقة الدفع',
  },
  'Pending qty. ${0}': {
    translation: 'الكمية المعلقة: ${0}',
  },
  Periodicity: {
    translation: 'الدورية',
  },
  Phone: {
    translation: 'الهاتف',
  },
  'Pick Columns': {
    translation: 'اختر الأعمدة',
  },
  'Pick Import Columns': {
    translation: 'اختر أعمدة الاستيراد',
  },
  Pink: {
    translation: 'وردي',
  },
  Place: {
    translation: 'مكان',
  },
  'Place of supply': {
    translation: 'مكان التوريد',
  },
  'Plants and Machineries': {
    translation: 'المصانع والآليات',
  },
  'Please Wait': {
    translation: 'يرجى الانتظار',
  },
  'Please add a customer before saving': {
    translation: 'الرجاء إضافة عميل قبل الحفظ',
  },
  'Please add items': {
    translation: 'الرجاء إضافة أصناف',
  },
  'Please check Key Hints for valid key names': {
    translation: 'يرجى التحقق من تلميحات المفاتيح للأسماء الصحيحة للمفاتيح',
  },
  'Please create a ${0} entry to view Template Preview.': {
    translation: 'يرجى إنشاء إدخال ${0} لعرض معاينة القالب.',
  },
  'Please fill all values.': {
    translation: 'يرجى ملء جميع القيم.',
  },
  'Please restart and try again.': {
    translation: 'يرجى إعادة التشغيل والمحاولة مرة أخرى.',
  },
  'Please select a Print Template': {
    translation: 'يرجى اختيار قالب للطباعة',
  },
  'Please select a batch first': {
    translation: 'الرجاء تحديد دفعة أولاً',
  },
  'Please select a customer': {
    translation: 'الرجاء اختيار العميل',
  },
  'Please select a payment method before proceeding with payment.': {
    translation: 'يرجى تحديد طريقة الدفع قبل متابعة الدفع.',
  },
  'Please select a payment method before submitting.': {
    translation: 'يرجى تحديد طريقة الدفع قبل الإرسال.',
  },
  'Please select a valid reference type.': {
    translation: 'يرجى اختيار نوع مرجع صالح.',
  },
  'Please set GSTIN in General Settings.': {
    translation: 'يرجى تعيين رقم GSTIN في الإعدادات العامة.',
  },
  'Please set Round Off Account in the Settings.': {
    translation: 'يرجى تعيين حساب التقريب في الإعدادات.',
  },
  'Please set a Display Doc': {
    translation: 'يرجى تعيين مستند للعرض',
  },
  'Point of Sale': {
    translation: 'نقطة البيع',
  },
  'Points must be greater than 0': {
    translation: 'يجب أن تكون النقاط أكبر من 0',
  },
  'Points must be greather than 0': {
    translation: 'يجب أن تكون النقاط أكبر من 0',
  },
  'Pos Print Width': {
    translation: 'عرض طباعة نقطة البيع',
  },
  'Pos Ui': {
    translation: 'واجهة نقطة البيع',
  },
  'Postal Code': {
    translation: 'الرمز البريدي',
  },
  'Postal Expenses': {
    translation: 'المصاريف البريدية',
  },
  'Posting Date': {
    translation: 'تاريخ النشر',
  },
  Prefix: {
    translation: 'بادئة',
  },
  'Price Discount': {
    translation: 'خصم السعر',
  },
  'Price Discount Scheme': {
    translation: 'مخطط خصم السعر',
  },
  'Price Discount Type': {
    translation: 'نوع خصم السعر',
  },
  'Price List': {
    translation: 'قائمة الأسعار',
  },
  'Price List Item': {
    translation: 'عنصر قائمة الأسعار',
  },
  'Pricing Rule': {
    translation: 'قاعدة التسعير',
  },
  'Pricing Rule Detail': {
    translation: 'تفاصيل قاعدة التسعير',
  },
  'Pricing Rule Item': {
    translation: 'عنصر قاعدة التسعير',
  },
  'Prime Bank': {
    translation: 'البنك الرئيسي',
  },
  Print: {
    translation: 'طباعة',
  },
  'Print ${0}': {
    translation: 'طباعة ${0}',
  },
  'Print Failed': {
    translation: 'فشلت الطباعة',
  },
  'Print Settings': {
    translation: 'إعدادات الطباعة',
  },
  'Print Setup': {
    translation: 'إعداد الطباعة',
  },
  'Print Successful': {
    translation: 'تمت الطباعة بنجاح',
  },
  'Print Template': {
    translation: 'قالب الطباعة',
  },
  'Print Template Name not set': {
    translation: 'اسم قالب الطباعة غير معيّن',
  },
  'Print Template is empty': {
    translation: 'قالب الطباعة فارغ',
  },
  'Print Templates': {
    translation: 'قوالب الطباعة',
  },
  'Print View': {
    translation: 'عرض الطباعة',
  },
  'Print and Stationery': {
    translation: 'طباعة و المكتبة',
  },
  Priority: {
    translation: 'أولوية',
  },
  Product: {
    translation: 'منتج',
  },
  'Product Discount': {
    translation: 'خصم المنتج',
  },
  'Product Discount Scheme': {
    translation: 'مخطط خصم المنتج',
  },
  Profile: {
    translation: 'الملف الشخصي',
  },
  'Profit And Loss': {
    translation: 'تقرير الربح والخسارة',
  },
  'Profit and Loss': {
    translation: 'الأرباح والخسائر',
  },
  Purchase: {
    translation: 'شراء',
  },
  'Purchase Acc.': {
    translation: 'حساب المبيعات',
  },
  'Purchase Amount': {
    translation: 'مبلغ الشراء',
  },
  'Purchase Invoice': {
    translation: 'فاتورة مشتريات',
  },
  'Purchase Invoice Item': {
    translation: 'عنصر فاتورة المشتريات',
  },
  'Purchase Invoice Number Series': {
    translation: 'تسلسل فاتورة المشتريات',
  },
  'Purchase Invoice Print Template': {
    translation: 'قالب الطابعة فاتورة المشتريات',
  },
  'Purchase Invoice Terms': {
    translation: 'شروط فاتورة المشتريات',
  },
  'Purchase Invoices': {
    translation: 'فاتورة مشتريات',
  },
  'Purchase Item': {
    translation: 'عنصر المشتريات',
  },
  'Purchase Item Created': {
    translation: 'تم إنشاء عنصر المشتريات',
  },
  'Purchase Items': {
    translation: 'عناصر المشتريات',
  },
  'Purchase Payment': {
    translation: 'دفعة المشتريات',
  },
  'Purchase Payment Account': {
    translation: 'حساب دفع المشتريات',
  },
  'Purchase Payments': {
    translation: 'مدفوعات المشتريات',
  },
  'Purchase Receipt': {
    translation: 'ايصال  الشراء',
  },
  'Purchase Receipt Item': {
    translation: 'عنصر إيصال الشراء',
  },
  'Purchase Receipt Location': {
    translation: 'موقع استلام المشتريات',
  },
  'Purchase Receipt Number Series': {
    translation: 'تسلسل  استلام المشتريات',
  },
  'Purchase Receipt Print Template': {
    translation: 'قالب الطابعة استلام المشتريات',
  },
  'Purchase Receipt Terms': {
    translation: 'شروط استلام المشتريات',
  },
  Purchases: {
    translation: 'المشتريات',
  },
  Purple: {
    translation: 'أرجواني',
  },
  Purpose: {
    translation: 'الغرض',
  },
  Qty: {
    translation: 'الكمية',
  },
  'Qty in Batch': {
    translation: 'الكمية في الدفعة',
  },
  'Qty. ${0}': {
    translation: 'الكمية ${0}',
  },
  'Qty. in Transfer Unit': {
    translation: 'الكمية في وحدة التحويل',
  },
  Quantity: {
    translation: 'الكمية',
  },
  'Quantity (${0}) has to be greater than zero': {
    translation: 'يجب أن تكون الكمية (${0}) أكبر من صفر',
  },
  'Quantity and Amount': {
    translation: 'الكمية والمبلغ',
  },
  'Quantity must be greater than zero.': {
    translation: 'يجب أن تكون الكمية أكبر من الصفر.',
  },
  'Quantity needs to be set': {
    translation: 'يجب تعيين الكمية',
  },
  Quarterly: {
    translation: 'ربع سنوي',
  },
  Quarters: {
    translation: 'أرباع',
  },
  'Quick Search': {
    translation: 'البحث السريع',
  },
  'Quick edit error: ${0} entry has no name.': {
    translation: 'خطأ في التحرير السريع: الإدخال ${0} ليس له اسم.',
  },
  Quotation: {
    translation: 'عرض سعر',
  },
  Quote: {
    translation: 'عرض سعر',
  },
  'Quote Reference': {
    translation: 'مرجع عرض السعر',
  },
  Rate: {
    translation: 'سعر',
  },
  'Rate (${0}) cannot be less zero.': {
    translation: 'لا يمكن أن يكون السعر (${0}) أقل من صفر.',
  },
  'Rate (${0}) has to be greater than zero': {
    translation: 'يجب أن يكون السعر (${0}) أكبر من صفر',
  },
  "Rate can't be negative.": {
    translation: 'لا يمكن أن يكون السعر سالبًا',
  },
  'Rate needs to be set': {
    translation: 'يجب تعيين السعر',
  },
  'Raw Value: ${0}': {
    translation: 'القيمة الخام: ${0}',
  },
  Receivable: {
    translation: 'مدين',
  },
  Receive: {
    translation: 'استلام',
  },
  Recent: {
    translation: 'الأحدث',
  },
  'Recurse Every (As Per Transaction Unit)': {
    translation: 'التكرار كل (حسب وحدة المعاملة)',
  },
  Red: {
    translation: 'أحمر',
  },
  'Redeem Loyalty Points': {
    translation: 'استرداد نقاط الولاء',
  },
  'Ref Name': {
    translation: 'اسم المرجع',
  },
  'Ref Type': {
    translation: 'نوع المصدر',
  },
  'Ref. / Cheque No.': {
    translation: 'المرجع. / رقم الشيك',
  },
  'Ref. Date': {
    translation: 'المرجع. التاريخ',
  },
  'Ref. Name': {
    translation: 'المرجع. الاسم',
  },
  'Ref. Type': {
    translation: 'المرجع. النوع',
  },
  Reference: {
    translation: 'مرجع',
  },
  'Reference Date': {
    translation: 'تاريخ المرجع',
  },
  'Reference Id not set.': {
    translation: 'لم يتم تعيين معرف المرجع.',
  },
  'Reference Number': {
    translation: 'الرقم المرجعي',
  },
  'Reference Type': {
    translation: 'نوع المرجع',
  },
  References: {
    translation: 'مراجع',
  },
  'Register Instance': {
    translation: 'تسجيل المثيل',
  },
  'Remove Filter': {
    translation: 'إزالة التصفية',
  },
  Replied: {
    translation: 'تم الرد',
  },
  Report: {
    translation: 'تقرير',
  },
  'Report Error': {
    translation: 'الإبلاغ عن خطأ',
  },
  'Report Issue': {
    translation: 'الإبلاغ عن مشكلة',
  },
  'Report will use more than one page if required.': {
    translation: 'سيستخدم التقرير أكثر من صفحة إذا لزم الأمر.',
  },
  Reports: {
    translation: 'التقارير',
  },
  'Required fields not selected: ${0}': {
    translation: 'الحقول المطلوبة غير محددة: ${0}',
  },
  'Retained Earnings': {
    translation: 'الأرباح المحتجزة',
  },
  Return: {
    translation: 'إرجاع',
  },
  'Return Against': {
    translation: 'إرجاع مقابل',
  },
  'Return Button Colour': {
    translation: 'لون زر الإرجاع',
  },
  'Return Issued': {
    translation: 'تم إصدار الإرجاع',
  },
  'Reverse Chrg.': {
    translation: 'الرسوم العكسية',
  },
  Reverted: {
    translation: 'تم إرجاعه',
  },
  Reverts: {
    translation: 'إرجاعات',
  },
  'Review Accounts': {
    translation: 'مراجعة الحسابات',
  },
  'Review your chart of accounts, add any account or tax heads as needed': {
    translation:
      'راجع دليل الحسابات الخاص بك، وأضف أي حساب أو بنود ضريبية حسب الحاجة',
  },
  'Right Index': {
    translation: 'الفهرس الأيمن',
  },
  Role: {
    translation: 'الدور',
  },
  'Root Type': {
    translation: 'نوع الجذر',
  },
  Round: {
    translation: 'تقريب',
  },
  'Round Free Item Quantity': {
    translation: 'تقريب كمية الصنف المجاني',
  },
  'Round Off': {
    translation: 'تقريب',
  },
  'Round Off Account': {
    translation: 'حساب التقريب',
  },
  'Round Off Account Not Found': {
    translation: 'لم يتم العثور على حساب التقريب',
  },
  'Rounded Off': {
    translation: 'مُقرّب',
  },
  'Rounding Method': {
    translation: 'طريقة التقريب',
  },
  'Row ${0}': {
    translation: 'صف ${0}',
  },
  Sa: {
    translation: 'السبت',
  },
  Salary: {
    translation: 'الراتب',
  },
  Sales: {
    translation: 'المبيعات',
  },
  'Sales Acc.': {
    translation: 'حساب المبيعات',
  },
  'Sales Expenses': {
    translation: 'مصاريف المبيعات',
  },
  'Sales Invoice': {
    translation: 'فاتورة مبيعات',
  },
  'Sales Invoice ${0} is Saved': {
    translation: 'تم حفظ فاتورة المبيعات ${0}',
  },
  'Sales Invoice ${0} is Submitted': {
    translation: 'تم تسجيل فاتورة المبيعات ${0}',
  },
  'Sales Invoice Item': {
    translation: 'اصناف فاتورة المبيعات',
  },
  'Sales Invoice List': {
    translation: 'قائمة فواتير المبيعات',
  },
  'Sales Invoice Number Series': {
    translation: 'تسلسل فاتورة المبيعات',
  },
  'Sales Invoice Print Template': {
    translation: 'قالب الطابعة فاتورة المبيعات',
  },
  'Sales Invoice Terms': {
    translation: 'شروط فاتورة المبيعات',
  },
  'Sales Invoices': {
    translation: 'فواتير المبيعات',
  },
  'Sales Item': {
    translation: 'عنصر المبيعات',
  },
  'Sales Item Created': {
    translation: 'تم انشاء صنف مبيعات',
  },
  'Sales Items': {
    translation: 'عناصر المبيعات',
  },
  'Sales Payment': {
    translation: 'دفعة المبيعات',
  },
  'Sales Payment Account': {
    translation: 'حساب دفع المبيعات',
  },
  'Sales Payments': {
    translation: 'مدفوعات المبيعات',
  },
  'Sales Quote': {
    translation: 'عرض سعر',
  },
  'Sales Quote Item': {
    translation: 'عنصر عرض السعر',
  },
  'Sales Quote Number Series': {
    translation: 'سلسلة أرقام عروض السعر',
  },
  'Sales Quote Print Template': {
    translation: 'قالب طباعة عرض السعر',
  },
  'Sales Quotes': {
    translation: 'عروض الأسعار',
  },
  'Sales and Purchase': {
    translation: 'المبيعات والمشتريات',
  },
  Save: {
    translation: 'حفظ',
  },
  'Save ${0}?': {
    translation: 'هل تريد حفظ ${0}؟',
  },
  'Save Button Colour': {
    translation: 'لون زر الحفظ',
  },
  'Save Customizations': {
    translation: 'حفظ التخصيصات',
  },
  'Save Template': {
    translation: 'حفظ النموذج',
  },
  'Save Template File': {
    translation: 'حفظ ملف النموذج',
  },
  'Save and Continue': {
    translation: 'حفظ ومتابعة',
  },
  'Save as PDF': {
    translation: 'حفظ كملف PDF',
  },
  'Save as PDF Successful': {
    translation: 'تم الحفظ كملف PDF بنجاح',
  },
  'Save changes made to ${0}?': {
    translation: 'هل تريد حفظ التغييرات التي أُجريت على ${0}؟',
  },
  'Save or Submit an entry.': {
    translation: 'احفظ أو سجّل إدخالًا.',
  },
  Saved: {
    translation: 'تم الحفظ',
  },
  'Search Item (Name or Barcode)': {
    translation: 'ابحث عن صنف (بالاسم أو الباركود)',
  },
  'Secured Loans': {
    translation: 'قروض مضمونة',
  },
  'Securities and Deposits': {
    translation: 'الأوراق المالية والودائع',
  },
  Select: {
    translation: 'تحديد',
  },
  'Select CoA': {
    translation: 'حدد شهادة توثيق البرامج',
  },
  'Select Color': {
    translation: 'تحديد اللون',
  },
  'Select Colour': {
    translation: 'اختر اللون',
  },
  'Select Country': {
    translation: 'تحديد الدولة',
  },
  'Select File': {
    translation: 'تحديد ملف',
  },
  'Select Image': {
    translation: 'حدد صورة',
  },
  'Select Template File': {
    translation: 'اختر ملف القالب',
  },
  'Select a Display Doc to view the Template': {
    translation: 'اختر مستندًا للعرض لعرض القالب',
  },
  'Select a Template type': {
    translation: 'اختر نوع القالب',
  },
  'Select a form type to customize': {
    translation: 'اختر نوع النموذج لتخصيصه',
  },
  'Select a pre-defined page size, or set a custom page size for your Print Template.':
    {
      translation:
        'اختر حجم صفحة محدد مسبقًا، أو عيّن حجم صفحة مخصص لقالب الطباعة الخاص بك.',
    },
  'Select column': {
    translation: 'اختر عمودًا',
  },
  'Select file': {
    translation: 'تحديد ملف',
  },
  'Select folder': {
    translation: 'تحديد مجلد',
  },
  'Select the Batch': {
    translation: 'حدد الدفعة',
  },
  'Select the template type.': {
    translation: 'اختر نوع القالب.',
  },
  Selected: {
    translation: 'محدد',
  },
  September: {
    translation: 'سبتمبر',
  },
  'Serial Number': {
    translation: 'الرقم التسلسلي',
  },
  'Serial Number ${0} does not belong to the item ${1}.': {
    translation: 'الرقم التسلسلي ${0} لا يخص الصنف ${1}.',
  },
  'Serial Number ${0} does not exist.': {
    translation: 'الرقم التسلسلي ${0} غير موجود.',
  },
  'Serial Number ${0} is not Active.': {
    translation: 'الرقم التسلسلي ${0} غير نشط.',
  },
  'Serial Number ${0} is not Inactive': {
    translation: 'الرقم التسلسلي ${0} ليس غير نشط.',
  },
  'Serial Number ${0} status is not Active.': {
    translation: 'حالة الرقم التسلسلي ${0} ليست نشطة.',
  },
  'Serial Number Description': {
    translation: 'وصف الرقم التسلسلي',
  },
  'Serial Number Series': {
    translation: 'سلسلة الأرقام التسلسلية',
  },
  'Serial Number Series cannot contain the following characters: /, ?, &, =, %':
    {
      translation:
        'لا يمكن أن تحتوي سلسلة الأرقام التسلسلية على الأحرف التالية: /، ?، &، =، %',
    },
  'Serial Number Status': {
    translation: 'حالة الرقم التسلسلي',
  },
  'Serial Number is enabled for Item ${0}': {
    translation: 'تم تفعيل الرقم التسلسلي للصنف ${0}',
  },
  'Serial Number is not enabled for Item ${0}': {
    translation: 'لم يتم تفعيل الرقم التسلسلي للصنف ${0}',
  },
  'Serial Number not set for row ${0}.': {
    translation: 'الرقم التسلسلي غير معيّن للصف ${0}.',
  },
  'Serial Number set for row ${0}.': {
    translation: 'تم تعيين الرقم التسلسلي للصف ${0}.',
  },
  Service: {
    translation: 'الخدمة',
  },
  'Set Coupon Code': {
    translation: 'تعيين رمز القسيمة',
  },
  'Set Discount Amount': {
    translation: 'تعيين مبلغ الخصم',
  },
  'Set Loyalty Program': {
    translation: 'تعيين برنامج الولاء',
  },
  'Set Period': {
    translation: 'تعيين الفترة',
  },
  'Set Price List': {
    translation: 'تعيين قائمة الأسعار',
  },
  'Set Print Size': {
    translation: 'تعيين حجم الطباعة',
  },
  'Set Template Type': {
    translation: 'تعيين نوع القالب',
  },
  'Set Up': {
    translation: 'إعداد',
  },
  'Set Up Your Workspace': {
    translation: 'إعداد مساحة العمل الخاصة بك',
  },
  'Set a Template value to see the Print Template': {
    translation: 'قم بتعيين قيمة القالب لرؤية قالب الطباعة',
  },
  'Set an Import Type': {
    translation: 'تعيين نوع الاستيراد',
  },
  'Set the display language.': {
    translation: 'ضبط لغة العرض.',
  },
  'Set the local code. This is used for number formatting.': {
    translation: 'اضبط الكود المحلي. يستخدم هذا لتنسيق الأرقام',
  },
  'Set up your company information, email, country and fiscal year': {
    translation:
      'قم بإعداد معلومات شركتك، البريد الإلكتروني، الدولة والسنة المالية',
  },
  'Set up your opening balances before performing any accounting entries': {
    translation: 'قم بإعداد أرصدة الافتتاح قبل تنفيذ أي قيود محاسبية',
  },
  'Set up your organization': {
    translation: 'إعداد شركتك',
  },
  'Set up your tax templates for your sales or purchase transactions': {
    translation: 'قم بإعداد قوالب الضرائب لمعاملات البيع أو الشراء الخاصة بك',
  },
  'Sets how many digits are shown after the decimal point.': {
    translation: 'يعين عدد الأرقام التي تظهر بعد العلامة العشرية.',
  },
  'Sets the app-wide date display format.': {
    translation: 'يضبط تنسيق عرض التاريخ على مستوى التطبيق.',
  },
  'Sets the internal precision used for monetary calculations. Above 6 should be sufficient for most currencies.':
    {
      translation:
        'يضبط الدقة الداخلية المستخدمة في الحسابات النقدية. الرقم الأعلى من 6 يجب أن يكون كافيًا لمعظم العملات',
    },
  'Sets the theme of the app.': {
    translation: 'يحدد سمة التطبيق.',
  },
  'Setting Up Instance': {
    translation: 'إعداد المثيل',
  },
  Settings: {
    translation: 'الإعدادات',
  },
  Setup: {
    translation: 'الإعداد',
  },
  'Setup Complete': {
    translation: 'اكتمل الإعداد',
  },
  'Setup Wizard': {
    translation: 'معالج الإعداد',
  },
  'Setup system defaults like date format and display precision': {
    translation:
      'إعداد الإعدادات الافتراضية للنظام مثل تنسيق التاريخ ودقة العرض',
  },
  Seven: {
    translation: 'سبعة',
  },
  Seventeen: {
    translation: 'سبعة عشر',
  },
  Seventy: {
    translation: 'سبعون',
  },
  Shipment: {
    translation: 'الشحنة',
  },
  'Shipment ${0} is Submitted': {
    translation: 'تم تسجيل الشحنة ${0}',
  },
  'Shipment Item': {
    translation: 'عنصر الشحنة',
  },
  'Shipment Location': {
    translation: 'موقع شحن البضاعة',
  },
  'Shipment Number Series': {
    translation: 'سلسلة أرقام الشحن',
  },
  'Shipment Print Template': {
    translation: 'قالب طباعة الشحنة',
  },
  'Shipment Terms': {
    translation: 'شروط الشحن',
  },
  Shortcuts: {
    translation: 'الاختصارات',
  },
  'Should entries be submitted after syncing?': {
    translation: 'هل يجب تسجيل الإدخالات بعد المزامنة؟',
  },
  'Show HSN': {
    translation: 'إظهار رمز HSN',
  },
  'Show Me': {
    translation: 'أرني',
  },
  'Show Month/Year': {
    translation: 'إظهار الشهر/السنة',
  },
  'Similar Product': {
    translation: 'منتج مماثل',
  },
  'Single Value': {
    translation: 'قيمة مفردة',
  },
  Six: {
    translation: 'ستة',
  },
  Sixteen: {
    translation: 'ستة عشر',
  },
  Sixty: {
    translation: 'ستون',
  },
  'Skip Child Tables': {
    translation: 'تخطي الجداول الفرعية',
  },
  'Skip Transactions': {
    translation: 'تخطي المعاملات',
  },
  'Smallest Currency Fraction Value': {
    translation: 'أصغر قيمة كسر للعملة',
  },
  Softwares: {
    translation: 'البرامج',
  },
  'Something has gone terribly wrong. Please check the console and raise an issue.':
    {
      translation:
        'لقد حدث خطأ ما بشكل رهيب. يرجى التحقق من وحدة التحكم وإثارة المشكلة',
    },
  'Source of Funds (Liabilities)': {
    translation: 'مصدر الأموال (الخصوم)',
  },
  'Standard Chart of Accounts': {
    translation: 'المخطط القياسي للحسابات',
  },
  Start: {
    translation: 'ابدأ',
  },
  'Start From Row Index': {
    translation: 'ابدأ من فهرس الصف',
  },
  State: {
    translation: 'الولاية',
  },
  'State Tax': {
    translation: 'ضريبة الولاية',
  },
  Status: {
    translation: 'الحالة',
  },
  Stock: {
    translation: 'المخزون',
  },
  'Stock Adjustment': {
    translation: 'تسوية المخزون',
  },
  'Stock Assets': {
    translation: 'أصول المخزون',
  },
  'Stock Balance': {
    translation: 'رصيد المخزون',
  },
  'Stock Entries': {
    translation: 'إدخالات المخزون',
  },
  'Stock Expenses': {
    translation: 'مصاريف المخزون',
  },
  'Stock In Hand': {
    translation: 'المخزون في متناول اليد',
  },
  'Stock In Hand Acc.': {
    translation: 'حساب المخزون في متناول اليد',
  },
  'Stock Ledger': {
    translation: 'دفتر الجرد الرئيسي للمخزون',
  },
  'Stock Ledger Entry': {
    translation: 'قيد دفتر أستاذ المخزون',
  },
  'Stock Liabilities': {
    translation: 'التزامات المخزون',
  },
  'Stock Movement': {
    translation: 'حركة المخزون',
  },
  'Stock Movement Item': {
    translation: 'عنصر حركة المخزون',
  },
  'Stock Movement No.': {
    translation: 'رقم تنقلات المخزون',
  },
  'Stock Movement Number Series': {
    translation: 'تسلسل تنقلات المخزون',
  },
  'Stock Movement Print Template': {
    translation: 'قالب الطابعة تنقلات المخزون',
  },
  'Stock Movements': {
    translation: 'حركات المخزون',
  },
  'Stock Not Received': {
    translation: 'المخزون غير مستلم',
  },
  'Stock Not Shipped': {
    translation: 'المخزون غير مُرسل',
  },
  'Stock Not Transferred': {
    translation: 'المخزون غير مُحوَّل',
  },
  'Stock Received But Not Billed': {
    translation: 'تم استلام المخزون ولكن لم يتم إصدار فاتورة به',
  },
  'Stock Received But Not Billed Acc.': {
    translation: 'حساب استلام المخزون من المورد ولم يتم إصدار فاتورة',
  },
  'Stock Transfer Item': {
    translation: 'عنصر تحويل المخزون',
  },
  'Stock Unit': {
    translation: 'وحدة المخزون',
  },
  StockTransfer: {
    translation: 'تحويل المخزون',
  },
  Stores: {
    translation: 'المخازن',
  },
  Su: {
    translation: 'الأحد',
  },
  Submit: {
    translation: 'تسجيل',
  },
  'Submit ${0}?': {
    translation: 'هل تريد تسجيل ${0}؟',
  },
  'Submit Button Colour': {
    translation: 'لون زر الإرسال',
  },
  'Submit entries?': {
    translation: 'هل تريد تسجيل الإدخالات؟',
  },
  Submitted: {
    translation: 'مُسجَّل',
  },
  Success: {
    translation: 'نجاح',
  },
  Supplier: {
    translation: 'المورد',
  },
  'Supplier Created': {
    translation: 'تم إنشاء المورد',
  },
  Suppliers: {
    translation: 'الموردون',
  },
  Symbol: {
    translation: 'الرمز',
  },
  'Sync Data From Server': {
    translation: 'مزامنة البيانات من الخادم',
  },
  'Sync Data To ERPNext': {
    translation: 'مزامنة البيانات إلى ERPNext',
  },
  'Sync Data To Server': {
    translation: 'مزامنة البيانات إلى الخادم',
  },
  'Sync Settings': {
    translation: 'إعدادات المزامنة',
  },
  System: {
    translation: 'النظام',
  },
  'System Settings': {
    translation: 'إعدادات النظام',
  },
  'System Setup': {
    translation: 'إعداد النظام',
  },
  Table: {
    translation: 'جدول',
  },
  Target: {
    translation: 'الهدف',
  },
  Tax: {
    translation: 'الضريبة',
  },
  'Tax Account': {
    translation: 'حساب الضرائب',
  },
  'Tax Amount': {
    translation: 'قيمة الضريبة',
  },
  'Tax Assets': {
    translation: 'الأصول الضريبية',
  },
  'Tax Detail': {
    translation: 'التفاصيل الضريبية',
  },
  'Tax ID': {
    translation: 'المعرّف الضريبي',
  },
  'Tax Invoice Account': {
    translation: 'حساب فاتورة الضريبة',
  },
  'Tax Payment Account': {
    translation: 'حساب دفع الضريبة',
  },
  'Tax Rate': {
    translation: 'معدل الضريبة',
  },
  'Tax Summary': {
    translation: 'ملخص الضريبة',
  },
  'Tax Template': {
    translation: 'قالب الضريبة',
  },
  'Tax Templates': {
    translation: 'قوالب الضريبة',
  },
  'Tax and Totals': {
    translation: 'الضرائب والإجماليات',
  },
  'Taxable Value': {
    translation: 'القيمة الخاضعة للضريبة',
  },
  'Taxed Amount': {
    translation: 'المبلغ الخاضع للضريبة',
  },
  Taxes: {
    translation: 'الضرائب',
  },
  'Taxes and Charges': {
    translation: 'الضرائب والرسوم',
  },
  Teal: {
    translation: 'أزرق مخضر',
  },
  'Telephone Expenses': {
    translation: 'مصاريف الهاتف',
  },
  Template: {
    translation: 'قالب',
  },
  'Template Builder': {
    translation: 'منشئ القوالب',
  },
  'Template Compilation Error': {
    translation: 'خطأ في تجميع القالب',
  },
  'Template Name': {
    translation: 'اسم القالب',
  },
  'Template Type': {
    translation: 'نوع القالب',
  },
  'Template file saved': {
    translation: 'تم حفظ ملف القالب',
  },
  Temporary: {
    translation: 'مؤقت',
  },
  'Temporary Accounts': {
    translation: 'حسابات مؤقتة',
  },
  'Temporary Opening': {
    translation: 'الافتتاح المؤقت',
  },
  Ten: {
    translation: 'عشرة',
  },
  Terms: {
    translation: 'الشروط',
  },
  'Terms and Conditions': {
    translation: 'الشروط والأحكام',
  },
  Text: {
    translation: 'نص',
  },
  Th: {
    translation: 'الخميس',
  },
  'The Grand Total must be less than ${0} to apply this coupon.': {
    translation: 'يجب أن يكون الإجمالي الكلي أقل من ${0} لتطبيق هذه القسيمة.',
  },
  'The Grand Total must exceed ${0} to apply the coupon ${1}.': {
    translation: 'يجب أن يتجاوز الإجمالي الكلي ${0} لتطبيق القسيمة ${1}.',
  },
  'The following characters cannot be used ${0} in a Number Series name.': {
    translation: 'لا يمكن استخدام الأحرف التالية ${0} في اسم سلسلة الأرقام.',
  },
  'The following items have insufficient quantity for Shipment: ${0}': {
    translation: 'العناصر التالية لا تحتوي على كمية كافية للشحن: ${0}',
  },
  Theme: {
    translation: 'السمة',
  },
  Thirteen: {
    translation: 'ثلاثة عشر',
  },
  Thirty: {
    translation: 'ثلاثون',
  },
  'This Month': {
    translation: 'هذا الشهر',
  },
  'This Quarter': {
    translation: 'هذا الربع',
  },
  'This Year': {
    translation: 'هذه السنة',
  },
  'This action is permanent': {
    translation: 'هذا العمل دائم',
  },
  'This action is permanent and will cancel the following payment: ${0}': {
    translation: 'هذا الإجراء نهائي وسيؤدي إلى إلغاء الدفعة التالية: ${0}',
  },
  'This action is permanent and will cancel the following payments: ${0}': {
    translation: 'هذا الإجراء نهائي وسيؤدي إلى إلغاء الدفعات التالية: ${0}',
  },
  'This action is permanent and will delete associated ledger entries.': {
    translation: 'هذا الإجراء دائم وسيؤدي إلى حذف قيود الأستاذ المرتبطة.',
  },
  'This action is permanent.': {
    translation: 'هذا الإجراء دائم.',
  },
  Thousand: {
    translation: 'ألف',
  },
  Three: {
    translation: 'ثلاثة',
  },
  'Tier Name': {
    translation: 'اسم الشريحة',
  },
  'Times New Roman': {
    translation: 'Times New Roman',
  },
  Title: {
    translation: 'عنوان',
  },
  To: {
    translation: 'إلى',
  },
  'To Account': {
    translation: 'إلى الحساب',
  },
  "To Account and From Account can't be the same: ${0}": {
    translation: 'لا يمكن أن يكون حساب المرسل إليه وحساب المرسل نفسه: ${0}',
  },
  'To Date': {
    translation: 'حتى تاريخ',
  },
  'To Loc.': {
    translation: 'إلى الموقع',
  },
  'To Year': {
    translation: 'حتى سنة',
  },
  'Toggle Edit Mode': {
    translation: 'تبديل وضع التحرير',
  },
  'Toggle Key Hints': {
    translation: 'تبديل تلميحات المفاتيح',
  },
  'Toggle Linked Entries widget, not available in Quick Edit view.': {
    translation:
      'تبديل أداة الإدخالات المرتبطة، غير متاحة في عرض التحرير السريع.',
  },
  'Toggle between Grid and List view': {
    translation: 'التبديل بين عرض الشبكة والقائمة',
  },
  'Toggle between form and full width': {
    translation: 'التبديل بين النموذج والعرض الكامل',
  },
  'Toggle sidebar': {
    translation: 'تبديل الشريط الجانبي',
  },
  'Toggle the Create filter': {
    translation: 'تبديل عامل التصفية للإنشاء',
  },
  'Toggle the Docs filter': {
    translation: 'تبديل عامل التصفية للمستندات',
  },
  'Toggle the List filter': {
    translation: 'تبديل عامل التصفية للقائمة',
  },
  'Toggle the Page filter': {
    translation: 'تبديل عامل التصفية للصفحة',
  },
  'Toggle the Report filter': {
    translation: 'تبديل عامل التصفية للتقرير',
  },
  'Top Expenses': {
    translation: 'أهم المصروفات',
  },
  Total: {
    translation: 'إجمالي',
  },
  'Total Amount': {
    translation: 'إجمالي المبلغ',
  },
  'Total Asset (Debit)': {
    translation: 'إجمالي الأصول (مدين)',
  },
  'Total Debit: ${0} must be equal to Total Credit: ${1}': {
    translation: 'إجمالي المدين: ${0} يجب أن يساوي إجمالي الدائن: ${1}',
  },
  'Total Discount': {
    translation: 'إجمالي الخصم',
  },
  'Total Equity (Credit)': {
    translation: 'اجمالى حقوق الملكية(دائن)',
  },
  'Total Expense (Debit)': {
    translation: 'اجمالى المصروفات(مدين)',
  },
  'Total Income (Credit)': {
    translation: 'اجمالى الدخل(دائن)',
  },
  'Total Liability (Credit)': {
    translation: 'اجمالى الالتزامات(دائن)',
  },
  'Total Profit': {
    translation: 'اجمالى الربح',
  },
  'Total Quantity': {
    translation: 'إجمالي الكمية',
  },
  'Total Spending': {
    translation: 'إجمالي الإنفاق',
  },
  'Track Inventory': {
    translation: 'تتبع المخزون',
  },
  Transfer: {
    translation: 'تحويل',
  },
  'Transfer No': {
    translation: 'رقم التحويل',
  },
  'Transfer Quantity': {
    translation: 'كمية التحويل',
  },
  'Transfer Type': {
    translation: 'نوع التحويل',
  },
  'Transfer Unit': {
    translation: 'وحدة التحويل',
  },
  'Transfer Unit ${0} is not applicable for Item ${1}': {
    translation: 'وحدة التحويل ${0} غير قابلة للتطبيق على الصنف ${1}',
  },
  'Transfer will cause future entries to have negative stock.': {
    translation: 'سيؤدي التحويل إلى أن تكون قيود المخزون المستقبلية سالبة.',
  },
  'Travel Expenses': {
    translation: 'مصاريف السفر',
  },
  'Trial Balance': {
    translation: 'ميزان المراجعة',
  },
  Tu: {
    translation: 'الثلاثاء',
  },
  Twelve: {
    translation: 'اثنا عشر',
  },
  Twenty: {
    translation: 'عشرون',
  },
  Two: {
    translation: 'اثنان',
  },
  Type: {
    translation: 'النوع',
  },
  'Type to search...': {
    translation: 'اكتب للبحث...',
  },
  UOM: {
    translation: 'وحدة القياس',
  },
  'UOM Conversion Item': {
    translation: 'عنصر تحويل وحدة القياس',
  },
  'UOM Conversions': {
    translation: 'تحويلات وحدة القياس',
  },
  'Unable to add an item to the return invoice.': {
    translation: 'تعذّر إضافة صنف إلى فاتورة الإرجاع.',
  },
  Unit: {
    translation: 'وحدة',
  },
  'Unit Type': {
    translation: 'نوع الوحدة',
  },
  Unpaid: {
    translation: 'غير مدفوع',
  },
  'Unpaid ${0}': {
    translation: '${0} غير مدفوعة',
  },
  'Unsecured Loans': {
    translation: 'قروض غير مضمونة',
  },
  'Until Date': {
    translation: 'حتى تاريخ',
  },
  'Use Full Width': {
    translation: 'استخدام العرض الكامل',
  },
  'Use List Filters': {
    translation: 'استخدام عوامل تصفية القائمة',
  },
  Used: {
    translation: 'مستخدم',
  },
  'User Remark': {
    translation: 'ملاحظة المستخدم',
  },
  'Utility Expenses': {
    translation: 'نفقات المرافق',
  },
  'Valid From': {
    translation: 'صالح من',
  },
  "Valid From Date should be greather than Pricing Rule's Valid From Date.": {
    translation:
      'يجب أن يكون تاريخ "صالح من" أكبر من تاريخ "صالح من" في قاعدة التسعير.',
  },
  'Valid From Date should be less than Valid To Date.': {
    translation: 'يجب أن تكون قيمة "صالح من تاريخ" أقل من "صالح حتى تاريخ".',
  },
  'Valid To': {
    translation: 'صالح حتى',
  },
  'Valid To Date should be greater than Valid From Date.': {
    translation: 'يجب أن تكون قيمة "صالح حتى تاريخ" أكبر من "صالح من تاريخ".',
  },
  "Valid To Date should be lesser than Pricing Rule's Valid To Date.": {
    translation:
      'يجب أن يكون تاريخ "صالح حتى" أقل من تاريخ "صالح حتى" في قاعدة التسعير.',
  },
  'Validation Error': {
    translation: 'خطأ في التحقق',
  },
  Validity: {
    translation: 'الصلاحية',
  },
  'Validity and Usage': {
    translation: 'الصلاحية والاستخدام',
  },
  Value: {
    translation: 'القيمة',
  },
  'Value missing for ${0}': {
    translation: 'القيمة مفقودة لـ ${0}',
  },
  'Value: ${0}': {
    translation: 'القيمة: ${0}',
  },
  Version: {
    translation: 'الإصدار',
  },
  View: {
    translation: 'عرض',
  },
  'View Accounting Entries': {
    translation: 'عرض القيود المحاسبية',
  },
  'View Paid Invoices': {
    translation: 'عرض الفواتير المدفوعة',
  },
  'View Purchases': {
    translation: 'عرض المشتريات',
  },
  'View Sales': {
    translation: 'عرض المبيعات',
  },
  'View Stock Entries': {
    translation: 'عرض إدخالات المخزون',
  },
  'View Unpaid Invoices': {
    translation: 'عرض الفواتير غير المدفوعة',
  },
  'View linked entries': {
    translation: 'عرض الإدخالات المرتبطة',
  },
  We: {
    translation: 'الأربعاء',
  },
  'Weight Enabled Barcode': {
    translation: 'باركود مفعّل بالوزن',
  },
  'When enabled, removes all default filters applied across the system, showing all available options instead of filtered ones.':
    {
      translation:
        'عند التمكين، يؤدي ذلك إلى إزالة جميع عوامل التصفية الافتراضية المطبقة عبر النظام، مما يؤدي إلى عرض جميع الخيارات المتاحة بدلاً من الخيارات التي تمت تصفيتها.',
    },
  'When linking documents, if no match is found and filtering is in effect, allow to disable filters.':
    {
      translation:
        'عند ربط المستندات، إذا لم يتم العثور على أي تطابق وكانت التصفية قيد التنفيذ، فاسمح بتعطيل عوامل التصفية.',
    },
  'Width (in cm)': {
    translation: 'العرض (بالسنتيمتر)',
  },
  'Write Off': {
    translation: 'شطب',
  },
  'Write Off Account': {
    translation: 'حساب الشطب',
  },
  'Write Off Account ${0} does not exist. Please set Write Off Account in General Settings':
    {
      translation:
        'حساب الشطب ${0} غير موجود. يرجى تعيين حساب الشطب في الإعدادات العامة',
    },
  'Write Off Account not set. Please set Write Off Account in General Settings':
    {
      translation:
        'شطب الحساب لم يتم تعيينه. يرجى تعيين حساب الشطب في الإعدادات العامة',
    },
  'Write Off Entry': {
    translation: 'قيد الشطب',
  },
  'Year to Date': {
    translation: 'منذ بداية السنة حتى اليوم',
  },
  Yearly: {
    translation: 'سنوي',
  },
  Years: {
    translation: 'سنوات',
  },
  Yellow: {
    translation: 'أصفر',
  },
  Yes: {
    translation: 'نعم',
  },
  Zero: {
    translation: 'صفر',
  },
  and: {
    translation: 'و',
  },
  'check values and click on': {
    translation: 'تحقق من القيم وانقر على',
  },
  field: {
    translation: 'حقل',
  },
  held: {
    translation: 'معلّق',
  },
  'in Batch ${0}': {
    translation: 'في الدفعة ${0}',
  },
  'item Group': {
    translation: 'مجموعة الصنف',
  },
  'john@doe.com': {
    translation: 'john@doe.com',
  },
  'no need ${0} points to purchase this item': {
    translation: 'لا تحتاج إلى ${0} نقطة لشراء هذا الصنف',
  },
  only: {
    translation: 'فقط',
  },
  'to apply changes': {
    translation: 'لتطبيق التغييرات',
  },
};

export default languageMap;
